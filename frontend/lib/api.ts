...
export async function fetchWrestlemaniaCard(
  playerToken?: string
): Promise<WMCardResponse> {
  const headers: Record<string, string> = {};
  if (playerToken) headers["x-player-token"] = playerToken;
  return apiFetch<WMCardResponse>("/api/wrestlemania/card", { headers });
}

export async function registerWMPlayer(
  displayName: string
): Promise<WMPlayer> {
  return apiFetch<WMPlayer>("/api/wrestlemania/players", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function submitWMPick(
  playerToken: string,
  matchId: number,
  outcomeId: number,
  stake: number
): Promise<{ id: number; outcome_id: number; stake: number; decimal_odds_at_pick: number }> {
  return apiFetch("/api/wrestlemania/picks", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-player-token": playerToken,
    },
    body: JSON.stringify({ match_id: matchId, outcome_id: outcomeId, stake }),
  });
}

export async function deleteWMPick(
  playerToken: string,
  pickId: number
): Promise<{ deleted: boolean }> {
  return apiFetch(`/api/wrestlemania/picks/${pickId}`, {
    method: "DELETE",
    headers: { "x-player-token": playerToken },
  });
}

export async function fetchWMLeaderboard(): Promise<WMLeaderboardResponse> {
  return apiFetch<WMLeaderboardResponse>("/api/wrestlemania/leaderboard");
}

export async function fetchWMOddsHistory(
  matchId: number
): Promise<WMOddsHistoryResponse> {
