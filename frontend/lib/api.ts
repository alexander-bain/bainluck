...
import { apiFetch, apiMutate, type PinsResponse } from "./apiCore";

/**
 * The transport and the landing page's own endpoints live in `./apiCore`
 * (LAT-P208) so `/` can load them WITHOUT this module — golf, admin, the themed
 * dashboards, onboarding and calibration are not on the first-card path and
 * this file is a single webpack module, so anything left here ships to every
 * route that touches it.
 *
 * They are re-exported so the ~65 call sites that import from `@/lib/api` are
 * unchanged. Modules on `/`'s EAGER graph must import from `@/lib/apiCore`
 * directly — importing them from here would drag this whole module back onto
 * the cold path, which is precisely the regression the entry-graph guard
 * (`__tests__/lib/emittedEntryGraph.test.ts`) exists to catch.
 */
export {
  API_URL,
  setAuthTokenGetter,
