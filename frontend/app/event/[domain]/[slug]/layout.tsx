...
/** The concept, or WHY there is none — #5861 needs the two apart. */
type ConceptLookup =
  | { ok: true; concept: EventConceptShareSource }
 ...
 */
async function fetchConcept(domain: string, slug: string): Promise<ConceptLookup> {
  if (!DOMAIN.test(domain) || !SLUG.test(slug)) return { ok: false, failure: "not-found" };
...
export async function generateMetadata({
  ...
  const lookup = await fetchConcept(domain, slug);
  ...
  if (!lookup.ok) {
    ...
    // 2026-09-13 08:50Z, a dead slug served `og:title: Event Odds` beside
