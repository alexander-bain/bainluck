...
import { fetchEIRankings } from "@/lib/api";
import type { RankedEvent } from "@/lib/types";
import EIBadge from "@/components/EIBadge";
import { getLeagueDisplay, getEmojiForLeague } from "@/lib/sportCategories";
import { usePageTracking, useScrollDepth, useEngagementTime } from "@/hooks";

/**
 * Excitement Index Hall of Fame Page
 *
 * Shows the all-time highest and lowest EI games ever tracked.
 */
export default function EIHallOfFamePage() {
  usePageTracking({ pageType: 'pulse_hof', pageTitle: 'Excitement Index Hall of Fame' });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-4 pb-6 border-b border-surface-border">
        ...
        <h1 className="text-title-1 text-text-primary">Excitement Index Hall of Fame</h1>
        ...
        <div className="space-y-3">
          {events.length === 0 ? (
            <div className="text-center py-12 text-text-secondary">
              No events with Excitement Index scores yet. Check back soon!
            </div>
          ) : (
            events.map((event) => {
              const eiData = event.ei || event.pulse;
              return (
              <Link
                key={event.id}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-shrink-0">
                      {eiData && (
                        <EIBadge ei={eiData} size="md" showTooltip={false} />
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="text-center pt-4 pb-8 space-y-4">
        ...
        >
          <span>&larr;</span>
          <span>Learn more about the Excitement Index</span>
        </Link>
        <div>
          <Link
            href="/"
