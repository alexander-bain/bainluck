...
/**
 * Line chart showing probability and projected score changes over time.
 */
export default function OddsChart({
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      const probPayload = payload.filter((p) => p.dataKey === "homeProb" || p.dataKey === "awayProb");
      const scorePayload = payload.filter((p) => p.dataKey === "homeScore" || p.dataKey === "awayScore");

      return (
        <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
          <p className="text-xs text-gray-500 mb-2">{label}</p>
          {probPayload.length > 0 && (
            <div className="mb-2">
              <p className="text-xs text-gray-400 mb-1">Win Probability</p>
              {probPayload.map((entry, index) => (
                <p
                  key={index}
          ...
          {showScores && scorePayload.length > 0 && (
            <div>
              <p className="text-xs text-gray-400 mb-1">Projected Score</p>
              {scorePayload.map((entry, index) => (
                <p
                  key={index}
                  className="text-sm font-semibold"
                  style={{ color: entry.color }}
