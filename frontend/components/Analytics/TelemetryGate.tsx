...
/**
 * LAT-P204 (#2673): the gated providers are LAZY, and that is the whole cut.
 *
 ...
 * But `import` is not `render`. Importing them statically here put gtag.js's
 * wrapper, the web-vitals reporter and the screen-timing reporter into the
 * BLOCKING entry chunk of every route, so a visitor who declined — or who has
 * not been asked yet, which is every cold first load — downloaded the code for
 * three rails they will never run before the first card could be drawn.
 *
 * `TelemetryGate` is a Client Component, so `dynamic()` here is a real split
 * point. That is not a given: the same calls written in `app/layout.tsx` would
 * split nothing, because a Server Component turns every client module it names
 * into a client reference of its own entry chunk and webpack has lost the split
 * point before `React.lazy` ever sees it (LAT-P200 / DeferredChrome.tsx).
 *
...
const ScreenTimingReporter = dynamic(() => import('./ScreenTimingReporter'), { ssr: false });
