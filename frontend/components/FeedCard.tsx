...
/**
 * Small "For You" indicator shown on personalized feed items.
 * Only renders when the item's score was boosted by personalization.
 */
function PersonalizedBadge({ item }: { item: FeedItem }) {
  if (!item.personalized) return null;

  // Translate internal personalization reasons to user-friendly labels
  const label = getPersonalizationLabel(item.personalization_reasons);

  return (
    <span
      className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded text-[10px] font-medium flex-shrink-0"
...
function getPersonalizationLabel(reasons?: string[]): string {
  ...
  // Check for the most meaningful reason and return a friendly label
