...
// — by DELETING `HT`. `dedupePeriodLabels` keeps the LATER of a too-close pair
// (its docstring: "so `HT` wins over `Q2 end`"), so here the survivor is `Q3` and
...
// one row. `assignPeriodLabelRows` runs AFTER `dedupePeriodLabels` and never
// changes the set — that separation is the first thing this file pins, because a
// regression that "fixed" the crowding by thinning would pass a naive stagger
// assertion while deleting the label the issue exists to protect.
//
// ── SPECIMEN ─────────────────────────────────────────────────────────────────
//
...
describe("#6882 — the specimen actually carries the collision", () => {
  ...
  it("the chart span is the one measured on the page, and nothing is collapsed at it", () => {
    ...
    expect(dedupePeriodLabels(b, CHART_SPAN_MS)).toHaveLength(b.length);
...
describe("#6882 — the stagger fires on exactly the crowded pair", () => {
  it("drops Q3 a row and leaves the other three on the top row", () => {
    const b = boundaries();
    const rowed = assignPeriodLabelRows(b, CHART_SPAN_MS);
  ...
  it("changes NOTHING about which markers are drawn", () => {
    ...
    // collapse wearing a different name.
    const b = boundaries();
    const kept = dedupePeriodLabels(b, CHART_SPAN_MS);
    const rowed = assignPeriodLabelRows(kept, CHART_SPAN_MS);

    expect(rowed.map((r) => r.label)).toEqual(kept.map((k) => k.label));
    expect(rowed.map((r) => r.timestamp)).toEqual(kept.map((k) => k.timestamp));
  });
  ...
  it("leaves a comfortably-spaced ladder entirely on the top row", () => {
    ...
    expect(
      assignPeriodLabelRows(roomy, span).map((r) => r.labelRow)
    ).toEqual([0, 0, 0, 0, 0, 0]);
  });

  it("alternates rather than running away when three markers are all crowded", () => {
    // Two rows are provably enough AFTER the collapse rule has run: every kept
    // pair is ≥7% apart, so A→C is ≥14%, already clear of the 12.6% band. This
    // pins that the implementation actually alternates instead of pushing a third
    // marker to row 2 — which the render has no offset for.
    const span = 100 * 60 * 1000;
    const crowded = [0, 8, 16, 24].map((m) => ({
      timestamp: new Date(Date.UTC(2026, 8, 18, 0, m, 0)).toISOString(),
      label: `M${m}`,
    }));
    const rows = assignPeriodLabelRows(crowded, span).map((r) => r.labelRow);
    expect(rows).toEqual([0, 1, 0, 1]);
    expect(Math.max(...rows)).toBeLessThanOrEqual(1);
  });

  it("the band sits strictly above the collapse threshold", () => {
    // A multiple ≤ 1 would make the stagger unreachable: anything tighter than the
    // collapse threshold has already been collapsed, so the band would be empty
    // and the fix inert on every page.
    ...
    expect(assignPeriodLabelRows(pair(justInside), span)[1].labelRow).toBe(1);
    expect(assignPeriodLabelRows(pair(justOutside), span)[1].labelRow).toBe(0);
  });
});
...
describe("#6882 — both components read the rule at their call sites", () => {
  ...
  it.each(CHARTS)("%s passes the row through to the label as dy", (_name, _render) => {
    ...
    expect(src).toMatch(/assignPeriodLabelRows\(/);
  });
...
describe("#6882 — what it does to a sport that is not football", () => {
  ...
  function mlbRowed(span: number) {
    ...
    return { kept: dedupePeriodLabels(b, span), span };
  }
  ...
  it.each(SPANS)("never needs a third row (span %ims)", (span) => {
    ...
    const rows = assignPeriodLabelRows(kept, span).map((r) => r.labelRow);
    expect(rows.length).toBeGreaterThan(0);
    expect([...new Set(rows)].every((r) => r === 0 || r === 1)).toBe(true);
  });

  it.each(SPANS)("drops no inning (span %ims)", (span) => {
    const { kept } = mlbRowed(span);
    expect(assignPeriodLabelRows(kept, span).map((r) => r.label)).toEqual(
      kept.map((k) => k.label)
    );
  });

  it.each(SPANS)("leaves no two labels sharing a row too close to read (span %ims)", (span) => {
    // The property the whole ship is for. Consecutive same-row labels are always
    // at least the stagger band apart — a label only drops when its predecessor
    // is on row 0, so two row-0 labels are separated by at least two collapse
    // thresholds (14%), which already clears the 12.6% band.
    const { kept } = mlbRowed(span);
    const rowed = assignPeriodLabelRows(kept, span);
    const band =
      span *
      PERIOD_LABEL_MIN_SPACING_FRACTION *
      PERIOD_LABEL_STAGGER_SPACING_MULTIPLE;
  ...
  it("the same invariant holds on the NFL specimen", () => {
    const rowed = assignPeriodLabelRows(boundaries(), CHART_SPAN_MS);
    const band =
      CHART_SPAN_MS *
      PERIOD_LABEL_MIN_SPACING_FRACTION *
      PERIOD_LABEL_STAGGER_SPACING_MULTIPLE;
...
describe("#6882 — the collapse rule it sits behind is untouched", () => {
  it("still keeps the LATER of a genuinely too-close pair", () => {
    // The stagger must not have been implemented by loosening this. If it were,
    // `HT` would come back at the cost of real collisions elsewhere.
  ...
  it("the collapse threshold has not moved", () => {
    // The named trap. Raising it is what deletes HT from every NFL chart.
