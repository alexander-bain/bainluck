...
describe("a settled field with many graded winners crowns nobody", () => {
  ...
  test("#6590's corrupt MUTEX board is declined by the same rule", () => {
    ...
    // no verdict beats a wrong one.
    // 🔴 `mutually_exclusive: true` is verified against the served payload, and it
    // is the DOOR this board comes through — its `market_type` is `duel`, which the
    // rule deliberately no longer treats as peer-shaped (2,620 of the 2,642
    // multi-winner `duel` boards are grouped independent markets whose crown is
    // correct). Exactly 22 declare exclusivity. This is one of them.
    const html = render(
      bmwPga({
        id: 60015154,
...
describe("gradedWinner counts grades on a peer-shaped board", () => {
  ...
  test("two grades ⇒ null", () => {
    ...
    expect(gradedWinner(o, o[0], "resolved", "participation")).toBeNull();
  });

  test("one grade ⇒ that row", () => {
    const o = rows(false, true, false);
    expect(gradedWinner(o, o[0], "resolved", "participation")).toBe(o[1]);
  });
  ...
  test("an unresolved board is null whatever the grades say", () => {
    ...
    expect(gradedWinner(o, o[0], "open", "participation")).toBeNull();
  });
  ...
  test("absent flags are not read as grades by omission", () => {
    ...
    expect(gradedWinner(o, o[0], "resolved", "participation")).toBe(o[2]);
  });

  test("🔴 `participation` is the ONLY shape that declines on plurality", () => {
    // The whole shape rule in one assertion, both directions. A shape added to the
    // classifier without a decision here shows up as a failure in this test.
    const o = rows(true, true);
    expect(gradedWinner(o, o[0], "resolved", "participation")).toBeNull();
    // 🔴 The four shapes the first draft of #7906 also withheld on. Each was read
    // against production and each keeps its crown — see `gradedWinner`'s comment
    // for the boards. `quantity` is the design exemption; `unshaped` and absent
    // are the refusal to guess. All of them keep the pre-#7906 answer.
    for (const shape of [
      "duel",
      "field",
      "claim",
      ...
      expect(gradedWinner(o, o[0], "resolved", shape)).toBe(o[0]);
    }
    expect(gradedWinner(o, o[0], "resolved")).toBe(o[0]);
  });

  test("🔴 an AFFIRMATIVE exclusivity claim declines whatever the shape says", () => {
    // #6590's door. A board that calls its outcomes exclusive and then grades two
    // of them has contradicted itself, and #4923 rules no verdict beats a wrong one.
    const o = rows(true, true);
    for (const shape of ["duel", "field", "quantity", "unshaped", undefined]) {
      expect(gradedWinner(o, o[0], "resolved", shape, true)).toBeNull();
    }
    // One grade on an exclusive board is the ordinary case and still crowns.
    const one = rows(false, true);
    expect(gradedWinner(one, one[0], "resolved", "duel", true)).toBe(one[1]);
  });

  test("🔴 only `true` decides — `false`/null/absent never withhold (CERT-609)", () => {
    // Polymarket's parser turns an ABSENT `negRisk` key into `false`, so a `false`
    // is not a claim of independence. Were this a truthiness test it would read the
    // same; were it `!== false` it would withhold on every Polymarket board alive.
    const o = rows(true, true);
    for (const flag of [false, null, undefined]) {
      expect(gradedWinner(o, o[0], "resolved", "duel", flag)).toBe(o[0]);
    }
    // ...and the shape door is not gated by the flag: the ship's own board is
    // `mutually_exclusive: false` and must still decline.
    expect(gradedWinner(o, o[0], "resolved", "participation", false)).toBeNull();
  });
});

/* ══════════ THE BOARDS THE FIRST DRAFT WOULD HAVE REGRESSED ══════════
 * Measured on production 2026-09-22 07:40Z. Each is `market_type: 'duel'`,
 * `mutually_exclusive: false`, two graded winners, and in each the crown we print
 * today is CORRECT — so the rule must leave it alone. These are not hypotheticals:
 * they are the served payloads of three live boards.
 * ════════════════════════════════════════════════════════════════════ */

describe("a grouped board of independent markets keeps its true crown", () => {
  const duelBoard = (id: number, name: string, legs: Array<[string, boolean]>) =>
    bmwPga({
      id,
      name,
      market_type: "duel",
      mutually_exclusive: false,
      outcomes: legs.map(([n, is_winner], i) => ({
        id: i + 1,
        name: n,
        probability: is_winner ? 1 : 0,
        rank: i + 1,
        is_winner,
      })),
    });

  test("🔴 61674035 — the match winner is named, not silenced by a side market", () => {
    // `Completed Match` is a SEPARATE Polymarket condition, not a rival claim.
    // Tiana Tian Deng won; both legs are legitimately true.
    const html = render(
      duelBoard(61674035, "W50 Nanao: Tiana Tian Deng vs Yuna Sugiyama", [
        ["Tiana Tian Deng", true],
        ["Completed Match", true],
      ]),
      "61674035",
    );
    expect(testIdText(html, "hero-resolved-name")).toBe("Tiana Tian Deng");
    expect(testIdText(html, "hero-resolved-chip")).toBe("Won");
  });

  test("🔴 61507880 — a cumulative round ladder still names its rung", () => {
    // Ends before round 2 ⇒ also ends before round 3. Plural grades are the design,
    // which is the exemption this guard's own comment argues for two paragraphs.
    const html = render(
      duelBoard(61507880, "Giga Chikadze vs Joanderson Brito: Round of Finish", [
        ["Fight ends before round 3", true],
        ["Fight ends before round 2", true],
      ]),
      "61507880",
    );
    expect(testIdText(html, "hero-resolved-name")).toBe("Fight ends before round 3");
  });

  test("🔴 112963 — a cumulative DATE ladder still names its rung", () => {
    const html = render(
      duelBoard(112963, "Will Russia capture all of Pokrovsk?", [
        ["Will Russia capture all of Pokrovsk by March 31", true],
        ["Will Russia capture all of Pokrovsk by February 28", true],
      ]),
      "112963",
    );
    expect(testIdText(html, "hero-resolved-name")).toBe(
      "Will Russia capture all of Pokrovsk by March 31",
    );
  });

  test("🔴 19490985 — a Yes/No board that graded BOTH crowns neither", () => {
    // The largest arm of this ship: 644 `container_member` boards declare
    // exclusivity and then grade both legs. Today the hero prints whichever the
    // payload emitted first — "Settled — No won." — which is a coin flip wearing a
    // verdict. The rows stay, so the contradiction is still visible below.
    const html = render(
      bmwPga({
        id: 19490985,
        name: "Set 1 Winner: Birrell vs Boulter",
        market_type: "container_member",
        mutually_exclusive: true,
        outcomes: [
          { id: 1, name: "No", probability: 1, rank: 1, is_winner: true },
          { id: 2, name: "Yes", probability: 1, rank: 2, is_winner: true },
        ],
      }),
      "19490985",
    );
  ...
  test("🔴 61821192 — a `field` board grouping one match's markets keeps its winner", () => {
    // 14,713 `field` boards are mixed and carry no discriminator; this one crowns
    // the real match winner today and the rule must not take it away.
    const html = render(
      bmwPga({
        id: 61821192,
        name: "Hangzhou Open, Qualification: Alex Bolt vs Omar Jasika",
        market_type: "field",
        mutually_exclusive: false,
        outcomes: [
          { id: 1, name: "Alex Bolt", probability: 1, rank: 1, is_winner: true },
          { id: 2, name: "Set 1 Winner", probability: 1, rank: 2, is_winner: true },
          { id: 3, name: "Omar Jasika", probability: 0, rank: 3, is_winner: false },
        ],
      }),
      "61821192",
    );
    expect(testIdText(html, "hero-resolved-name")).toBe("Alex Bolt");
  });
});
