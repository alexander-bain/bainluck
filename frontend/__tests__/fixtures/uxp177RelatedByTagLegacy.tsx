/**
 * UX-P177 — the VERBATIM pre-fix `components/RelatedByTag.tsx`, kept so the
 * BEFORE panel of `artifacts-ux-p177/related-by-tag-states.html` is a render of
 * the code that actually shipped, not a drawing of it.
 *
 * Extracted with `git show 1668221c:frontend/components/RelatedByTag.tsx`. The
 * ONLY edits are this header and the renamed default export; every line of the
 * row logic — including the `tournament`-only skip and the
 * `` `/futures/${d.id}` `` fallthrough that produced `/futures/undefined` — is
 * untouched.
 *
 * Precedent: `playerPropsGroupingLegacy.ts` in this same directory.
 *
...
export default function RelatedByTagLegacy({
  tags,
  excludeId,
  excludeType,
  limit = 6,
