/**
 * LAT-P204 (#2673) — the analytics barrel is on the blocking entry path, and
 ...
 *
 *  1. `components/Analytics/index.ts` re-exported six symbols, and fourteen
 ...
 *     gtag.js wrapper rode into the entry chunk of every route behind one
 *     React hook.
 ...
 *
 * WHAT MOVED SINCE THE FIRST DRAFT (latency/854, 2026-09-20). The gated set is
 * read off the tree, not inherited: `@vercel/analytics/next` left this file
 * under Alex ruling D96 (it is cookieless, so it is mounted ungated elsewhere)
 * and `ScreenTimingReporter` joined it. The guard names the three that are
 * actually here; adding a fourth gated provider without lazily importing it
 * reds the `GATED`-vs-tree consistency check below.
 */
...
/** Every `{decision.foo && <` mount the gate actually contains. */
function gatedMountKeys(code: string): string[] {
  const keys: string[] = [];
  for (const [, key] of code.matchAll(/\{\s*(decision\.[A-Za-z0-9_]+)\s*&&\s*</g)) {
    keys.push(key);
  }
  return keys;
}
...
describe('the analytics barrel carries nothing a caller does not ask it for', () => {
  ...
  it('the components only ONE place mounts are not re-exported here', () => {
    // These each have exactly one mount site, and each of those sites imports
    // the module directly. Re-exporting any of them puts it back into the entry
    // chunk of all fourteen `useAnalyticsContext` callers.
    for (const name of [
      'TelemetryPreferences',
      'GoogleAnalytics',
      'WebVitalsReporter',
      'ScreenTimingReporter',
    ]) {
...
describe('a declined visitor does not download the rails they declined', () => {
   ...
   *
   * `WebVitalsReporter` and `ScreenTimingReporter` share `decision.webVitals`
   * deliberately (latency/121): same class of page-performance metric, same
   * grant. That is a property of the consent design, not a copy-paste slip.
   */
  const GATED: Array<[string, string]> = [
    ['./GoogleAnalytics', 'decision.googleAnalytics'],
    ['./WebVitalsReporter', 'decision.webVitals'],
    ['./ScreenTimingReporter', 'decision.webVitals'],
  ];
   ...
   * against the pre-split tree this test passes while the rest fail, which is
   * what makes those a detector and not a tautology. An earlier draft also
   * asserted `next/dynamic` here; that is arm-dependent — it is part of the
   * CLAIM, not a control — so it now lives with the claim.
   */
  ...
  it('GATED lists every provider the gate actually mounts — no fourth rail sneaks in static', () => {
    // The table above is hand-written, so it can go stale the moment someone
    // adds a provider. This is the check that notices: the number of gated JSX
    // mounts in the file must equal the number of entries here. A new rail that
    // is mounted behind a decision but imported statically reds here even
    // though every assertion above still passes.
    const mounts = gatedMountKeys(code);
    if (mounts.length === 0) {
      throw new Error(
        'analyticsBarrelEntryCost: parsed ZERO `{decision.x && <` mounts out of ' +
          'TelemetryGate.tsx. The gate renders its providers conditionally; finding ' +
          'none means this guard cannot read the file.',
      );
    }
    expect(mounts.length).toBe(GATED.length);
    expect(new Set(mounts)).toEqual(new Set(GATED.map(([, key]) => key)));
  });
