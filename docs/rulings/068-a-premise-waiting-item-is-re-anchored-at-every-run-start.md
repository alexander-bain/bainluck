# RULING 068 — A premise-waiting item is RE-ANCHORED at every run start

date: 2026-08-14
author: Alex
issues: #1801, #1501

> **Renumbered 053 → 068 (banked 2026-08-15, queue 357).** This ruling was written to disk on
> `lane1/q351` as `053-a-premise-waiting-item-is-re-anchored-at-every-run-start.md` **without a
> ledger claim**, and collided with `ux-63`'s 053 (*a binary card leads with the side its own
> question names*), which did claim and is merged on master. Per Fable's INT-070 ruling — banked as
> [069](069-the-ledger-is-the-allocator.md), *the ledger is the allocator: a number is held by the
> first CLAIM, not by the first merge* — ux keeps 053 and this one moves. 068 was reserved for it in
> `RULING-CLAIMS.md` on 2026-08-14 and held by nothing on any of the 330 refs swept at that time.
> Recorded here rather than resolved silently, per [055](055-a-conflict-resolution-that-changes-a-decision-is-a-decision.md).
>
> Note also *which* number the reservation had to be. The directive said "the next ledger-claimed
> free number (≥ 067)"; 067 was already gone by the time the Integrator measured, taken mid-cycle by
> `ux-69`. That is the ledger's own *floor, not oracle* header paying out — inside the very cycle
> that ruled on it.
