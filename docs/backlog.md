...
5. Automate the Polymarket email-highlight ground truth pipeline:
   ...
   - Next social-ground-truth work: add an offline image/caption extraction pipeline that accepts local screenshots/exports, uses vision/OCR to propose `{source, platform=instagram, handle, name, hook, url, published_at}`, writes reviewable JSONL/CSV, and feeds the existing advisory diagnostics. Add an admin review step before any social-derived row affects ranking metrics.
   - Next persistence work: persist reviewed curator rows.
