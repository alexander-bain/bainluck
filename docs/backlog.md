...
5. Automate the Polymarket email-highlight ground truth pipeline:
   ...
   - ✅ Admin health now breaks down Polymarket email row filtering by CSV rows, Polymarket source rows, old rows, low-interestingness rows, duplicates, loaded rows, source latest date, loaded latest date, and cutoff date. This makes the `39/538` style warning actionable instead of opaque.
   - ✅ Repo Apps Script source now preserves the legacy `Email Blurbs (archive)` append while also writing stable backend-readable rows into `Audit Export` with canonical headers, deterministic category/hook/interestingness/timeliness/shareability fields, and duplicate protection.
   ...
   - Automate the old Kalshi/Polymarket browse-page sweep as an advisory weekly regression check, not daily ranking training at first. Existing static files are `backend/scripts/kalshi_ground_truth.json` and `backend/scripts/polymarket_browse_ground_truth.json`; a future workflow should capture only top/trending browse sections, normalize to JSONL with source/page/rank/market/probability, compare against Discover top 50, and upload artifacts/admin diagnostics without automatic score changes.
...
3. ~~**Flag suppressed effectively-settled markets for ops follow-up** — suppression protects the feed, but the market should also appear in diagnostics so we can investigate why the exchange/source did not resolve cleanly.~~ ✅ Shipped May 21 via `/api/admin/discover-quality/effective-settlement-followups`. Issue: [#465](https://github.com/alexander-bain/bainluck/issues/465)
...
**Current status:** Probability-based hardening and admin follow-up diagnostics have shipped via options 1-3. The remaining deeper fix is option 4 for explicit playoff-series truth.
...
**May 21 decision gate:** Added `docs/search-fts-runbook.md` and `scripts/audit_search_fts_readiness.py` so issue #447 can be decided from production traces plus optional read-only `EXPLAIN ANALYZE` evidence. No migration/triggers until the runbook criteria are met.
