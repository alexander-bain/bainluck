...
5. Automate the Polymarket email-highlight ground truth pipeline:
   ...
   - ✅ First external-curator source lane shipped as an advisory local-input parser: `EXTERNAL_CURATOR_GROUND_TRUTH_PATHS` accepts CSV/JSON/JSONL exports, keeps only public URL metadata, performs no scraping/network calls, dedupes rows, and feeds audit/admin debug alongside email highlights. Next: persist reviewed curator rows and add source health/freshness diagnostics.
6. Add persisted matching diagnostics for email-highlight rows: matched `futures_markets.id`, current Discover rank, score bucket, missing reason, category/story family, and whether the card had usable image/context/explanation treatment. May 18 slice: added `discover_ground_truth_diagnostics` plus `scripts/snapshot_discover_ground_truth_diagnostics.py` to persist current debug API hit/miss rows for combined, email, and external-curator ground truth under a run ID. Next: schedule this after the email/curator import refresh and add a compact admin trend panel.
