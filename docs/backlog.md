...
5. Automate the Polymarket email-highlight ground truth pipeline:
   ...
   - Add an external-curator source lane after the email pipeline is stable: track 3-5 public market-curation feeds/newsletters/social accounts that consistently surface interesting Kalshi/Polymarket markets, ingest only stable public URLs or manually reviewed exports at first, dedupe them into the same ground-truth table, and keep them advisory until false positives are reviewed. Do not scrape logged-in/private feeds or depend on fragile social DOM scraping; prefer RSS, email/newsletter archives, platform APIs, or a simple manual CSV import.
