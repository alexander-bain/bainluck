...
>
> **`[L2-157]` INDEX/OUTCOME-PALETTE HALF RESOLVED — class E fully closed.** The series palettes now live in ONE registry, `frontend/lib/seriesColors.ts` (`SERIES_COLORS` + `SERIES_COLORS_GOLD`/`_GREEN` themes + `ELIMINATED_SERIES_COLOR`/`COMBINED_SERIES_COLOR`). Of the census's five, `POSITION_COLORS`/`TournamentChart` had already been deleted (dead by L2-157); the live drift was the generic index palette disagreeing with itself — the FuturesChart kernel drew a blue-led rainbow (`DEFAULT_COLORS`) while EvolutionView/EvolutionLeaderboard drew a crimson-led one (`EVOLUTION_COLORS`, defined identically in two files) for the same "Nth competitor" role. Canonicalization rule = the flagship series surface wins: the shared FuturesChart field kernel (renders the event-page hero charts) keeps its 8 hexes verbatim as `SERIES_COLORS[0..7]`, extended to 10 for many-competitor headroom using two hues already present in the old EVOLUTION palette (`#92400e`, `#065f46`) — no new colors invented. GOLD/GREEN are distinct leader THEMES (no conflict), migrated unchanged. Migrated + local copies deleted: FuturesChart, EvolutionView, EvolutionLeaderboard (index palette + inline eliminated-grey). Visible shift: EvolutionView/EvolutionLeaderboard series adopt the kernel palette (blue-led) — the event-page hero charts are unchanged. Guard: `__tests__/lib/seriesColors.test.ts` asserts indices 0–7 = flagship, headroom hues reused not invented, neutrals outside the live palette.
