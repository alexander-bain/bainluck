...
- **No accuracy claims, and no claims about our whole record. Ever.** (Alex, 2026-09-01: *"I'd generally prefer not to broadly claim amazing accuracy… just write better copy… Why don't we just stop making broad claims in the first place?"*) The product never tells a reader how accurate it is, how accurate it has been, or what has or has not happened across all of history. Not in marketing copy, not in an empty state, not in a tooltip.
  - Banned outright: "the most accurate", "consistently accurate", "we've never been wrong", "our track record", "always right", and any percentage offered as a claim about *us* rather than about a market.
  - Banned in the same breath: the **absolute quantifier over our own record** — "never", "ever", "at no point", "not once", "there has never been" — applied to a number reaching us or a question being answered. We hold the latest observation, not the archive, so a sentence of that shape is one the payload cannot support even when it happens to be true.
  - **Write what we HAVE, in the present.** "We have no number for X yet, so this comparison is not complete" — not "No number ever reached us for X, so this comparison was never complete." Same fact, and only the first one is something we can know.
  - The calibration page is the one place accuracy is *reported*, and it reports measured buckets with their sample sizes. That is a table, not a claim; it says what happened to N markets, never how good we are.
  - **This rule is enforced by review, not by a regex, and that is deliberate.** `frontend/lib/copyBans.ts` carries a literal blocklist of the sentences we are known to have served — it is a regression guard against a revert, nothing more. Six attempts to teach a pattern to *recognise* a history claim were rejected six times for failing ordinary true sports prose ("The comeback was never complete", "We never had a chance after halftime"). A person reading the sentence is the guard.
