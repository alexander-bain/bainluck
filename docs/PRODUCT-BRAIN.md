...
- [141](rulings/141-venue-names-are-not-part-of-the-product-a-reader-bought.md) — 2026-08-28 — VENUE NAMES ARE NOT PART OF THE PRODUCT A READER BOUGHT. Alex's words: "venue names — Kalshi and Polymarket — are BANNED in user-facing copy, everywhere. Users get our probability, not our sourcing." Permanent and product-wide, issued from the same live-page reading as rulings 138 and 142. This is ruling 138's clause one level up: 138 banned the trading layer's noun, this bans its proper nouns. A reader shown one blended number and then told which two exchanges it came from has been handed the plumbing back after we removed it, and handed a decision the product exists to make for them — the standing "the blend is the product" ruling, applied to the caption. THE SHARPER REASON: naming the venue makes an absence sound like a supplier problem. The grid legend said "we asked Kalshi and Polymarket and neither runs that market", which tells a reader we have two suppliers and both let us down; what is true is that NOBODY is answering that question anywhere, which is both shorter and more useful. GENERAL FORM: a translated surface may not name the sources it translated — where the abstraction is the product, what sits under it is implementation detail, and putting it in front of a reader converts a finished answer back into a research task. DOES NOT TOUCH source ids (`source: "kalshi"`, `group_id: "polymarket:12345"`, `stale_sources`, every enum and data attribute — the guard is CASE-SENSITIVE precisely so the lowercase ids pass and the capitalised names do not), code, comments, `/admin` (an operator needs the exact venue and ticker), `/privacy` (a legal disclosure must name who we read), or deliberate comparison surfaces (`/calibration` publishes how well each SOURCE predicts; a source-accuracy table with the sources anonymised is not a stronger version of itself — LISTED as debt rather than exempted, because that is Alex's call and not a sweep's). THE HONESTY SURVIVED BOTH REMOVALS, pinned in both directions: the no-market legend keeps "the question is unanswered, not unread", and the muted-row line went from "Polymarket 20 days ago" to "one reading 20 days ago" — the venue name never carried that admission, the PARTITIONING did, and a count says "only part of this is old" identically, in the page's own existing "no reading yet" vocabulary. SWEPT by UX-P150 on the tournament surfaces; OWED and recorded as executable debt in the `OWED` map (keyed on surface AND rule, so an unlisted surface fails and a new kind of violation on a listed surface also fails): `/calibration`, `/weather`, `/politics`, `/categories/golf`, `/about`, the shared landing blurbs, `CombinedFeedCard` + `SourceComparisonRow` (closest to "the blend is the product" and the next one that should go), and `app/layout.tsx`'s meta keywords (not visible to a reader, so out of scope on this ruling's own terms, but flagged to Alex as an SEO call rather than a copy one) (Alex directive authored in Alex's Fable session 2026-08-28, delivered through the lane runner Alex launched under his standing authorization)
