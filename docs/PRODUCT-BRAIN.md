...
## THE FOUR PILLARS — the constitution (Alex, 2026-08-28) — DO NOT REMOVE (CI-guarded)
**These four are supreme.** Every codebase decision, every architecture decision, every UX decision answers to them. Where anything else — a section further down this file, a line in `CLAUDE.md`, a queue brief, a lane's habit, an agent's taste — conflicts with a pillar, the pillar wins and the other thing is the bug. They are not priorities to be traded off against each other by whoever is holding the keyboard; they are what the product IS, stated so that a decision can be checked against something.

1. **MATCHING.** Every meaningful source, perfectly semantically matched — the one-stop shop for probabilities. Not "most sources, mostly matched": the pillar is the *perfectly*, because a user who has to go somewhere else for one question has learned that they might have to go somewhere else for any question. This is why semantic matching is the #1 technical challenge in `CLAUDE.md` and why an unmerged duplicate is a reliability failure rather than a cosmetic one.
2. **DISCOVER.** Learn what is interesting — globally and per user — from accumulated signals. The feed is not a query result and it is not an editor's list; it is a thing that gets better because we kept the signals and used them. "Interesting" is learned, not hand-tuned, and the per-user half is as load-bearing as the global half.
3. **FORMATTING.** Related probabilities elegantly connected, movement glanceable, Hipmunk/Kayak-grade UX force. The comparison is the instruction: those products won on the *presentation* of information everyone else already had. A number nobody can read at a glance has not been delivered, and a set of related numbers presented as unrelated rows is a formatting failure, not a data one.
4. **TRUTH.** The aggregate is calibrated. **"Probability, not betting" is a promise** — the whole positioning rests on the number being what it says it is, so calibration is not a lab metric we publish, it is the thing the promise is made of. A miscalibrated aggregate is a broken product even on a day when every page renders.

**Provenance, stated exactly (doctrine: an attribution is a claim about evidence).** Alex articulated the four pillars on **2026-08-28**; they reached this repo through the lane1 runner directive staged by Fable that same day, carrying Alex's review-and-authorize attestation. The pillar wording above is that directive's wording, expanded here with the WHY this file exists to carry — the expansions are this lane's reading, not Alex's sentences. **One sentence IS verbatim Alex** and is quoted as such in `CLAUDE.md`'s queue law: *"I don't want to spend several more months creating beautiful architecture while simultaneously being embarrassed that I still don't have a product to show anyone."*

**How supremacy is actually exercised** — it is not a preamble, it has a mechanism. `CLAUDE.md`'s QUEUE LAW binds every queue to name the pillar it serves and the user-visible ship it delivers, and confines architecture work to the substrate of a ship already queued (the RIDER RULE). That is where a pillar stops being a value and starts being a gate: a queue that cannot name its pillar does not run, and a program whose only output is architecture is forbidden regardless of how good the architecture is. The pillars are the WHAT; `CLAUDE.md`'s PROGRESS, NOT MEASUREMENT section is the HOW; ruling 134 governs which lane may measure.

## HOW TO USE THIS
The queue-file mechanics live in `.claude/handoff/README.md` + `FABLE-STANDIN.md`. THIS doc is the judgment layer: the standing rulings, the reasoning behind them, and the failure modes to avoid. **The ordered backlog is [GitHub Issues](https://github.com/alexander-bain/bainluck/issues) and nothing else** — priority, status and completion live there, per "Board = the SOLE record" below. `plan_next_10_queues.md` is a retired local snapshot and is not consulted for ordering. Pre-written specs are in `READY-FIXES.md`; deep strategy in `.claude/handoff/strategy_*.md`.
...
- [009](rulings/009-precompute-calibration-freeze.md) — 2026-08-09 — precompute_calibration.py is frozen until the publish converges (Alex). **AMENDED 2026-08-28 (Alex, MC, #2248): the lift condition's clause 2 changes shape from "~13 CONSECUTIVE clean beats" to "22 OF THE LAST 24 beats publish cleanly", and the baseline moves from CAL-P024 to the deploy carrying the CAL-P109/P110 phase-budget repair.** The intent is untouched — a known-good producer runs undisturbed long enough to prove it converges — and the freeze is NOT lifted by the amendment. N/M chosen from the measured record, not picked: over the 166 pre-fix beats in `calibration:beat_gauge_history` (2026-08-21T18:37Z → 2026-08-28T15:34Z, publish rate 0.476, longest clean run 9, best 24-window 19/24) the per-window probability of 22-of-24 is **5.6 × 10⁻⁶ at the broken 0.472 rate** and **0.884 at a healthy 0.95 rate** (≈1 day expected wait) — comfortably reachable when the producer works, effectively unreachable while it loses half its beats. THE SHAPE CHANGE BUYS DISCRIMINATION, NOT SPEED, and the amendment says so: at 0.95 the original condition actually closes FASTER (0.79 d vs 1.04 d). What it buys is measured under a clustering-aware moving-block bootstrap of the real pre-fix sequence, where the broken producer satisfies 13-consecutive with probability **0.27–0.59 in 90 days** but 22-of-24 with only **0.006–0.105** — a streak counter is a poor test of a rate, because clustered misses leave long clean stretches behind them. 22 is the first value the measured pre-fix process does not reach (20-of-24 → ~1.00, 21-of-24 → 0.48–0.70); 23-of-24 was rejected in the other direction as re-creating a softer deadlock. M=24 is one day of the hourly beat, so the condition reads without arithmetic: one full day in which the producer lost at most two. CORRECTS THIS PROGRAM'S OWN PUBLISHED FIGURE: #2248's "1 in 18,561, ~2 years" is an i.i.d. artifact — the deadlock is real but its mechanism is the observed rate, not an astronomically small probability. Made executable rather than left in prose (gotcha #35) as `backend/scripts/calibration_freeze_score.py`. Does NOT sanction the 842 lines already written into the frozen file on `program/calibration-99` (#2248 option 3 was not taken — they become mergeable when the lift is observed and recorded, and not before), does NOT lift the freeze, does NOT weaken the Alex-escalation path, and does NOT touch the reliability bar or cert tiering (ruling 133)
...
- [132](rulings/132-capture-plays-fix-first.md) — 2026-08-24 — CAPTURE PLAYS FIX-FIRST: a salvage batch run through a known-broken planner spends the thing it is trying to save. The settlement-truth program runs no salvage batches while a filed planner defect is unfixed; the order is fix -> cert -> full RUN, and as ruled tonight that means the #2175 livelock fix plus the PRE-STAGED CERT-399 acceptance (G1-G7, already in CERT-QUEUE.md) then a full RUN 2026-08-25. WHY THE USUAL DEADLINE INSTINCT IS INVERTED HERE: a sweep's budget is not money or time but PROBE ATTEMPTS AGAINST ROWS THAT EXPIRE, so an attempt spent on the wrong row is not wasted-and-neutral, it is subtracted from the rows that could still have answered — a broken run is a transfer of budget from the saveable rows to whichever rows the planner happens to prefer. Two measured defects made that concrete: the `ambiguous_empty` livelock (#2175) reserved the head of the terminal ordering for rows that had already said they cannot answer, and the `overdue` starvation found while folding in the retention measurement sorted the cohort with the LEAST life left LAST, so a binding budget never reached it. THE SECOND REASON OUTLIVES THE DEADLINE: a salvage batch also destroys the evidence that the fix worked, because the next planner reads probe histories, dispositions and `attempts` counters that the previous run's BUG wrote — a cert asked afterwards cannot separate 'the fix works' from 'the population was already churned', and the pre-fix measurement cannot be re-taken. Ruling 050's shape applied to a population instead of a metric: once you have mutated the thing you were going to measure, the measurement is not pending, it is lost. BINDS: filed-and-diagnosed is enough to stop a run (it need not be reproduced in production first); the acceptance criteria are the ones written BEFORE the fix existed, and the author does not get to restate the ruler; the RUN gates on cert GREEN and not on the calendar; the post-GREEN sequence (fresh census -> Alex's attended kill-mid-batch demo -> RUN) belongs to whoever holds it, since fixing a planner confers no authority to fire the sweep; and a deadline is a reason to fix faster, never a reason to run broken — moving the date is reversible, spending the attempts is not. DOES NOT forbid read-only work under a known defect (censuses and planner rehearsals are how the `overdue` starvation was FOUND), and does not generalise to deadlines whose population is stable and whose retries are free. Qualifying test: does a wasted attempt reduce the attempts available to the rows that could still succeed? (Alex ruling, Q405 addendum 2, pasted and reviewed by Alex)
- [133](rulings/133-cert-depth-tiers-by-blast-radius.md) — 2026-08-24 — CERT DEPTH TIERS BY BLAST RADIUS: full adversarial cert is for code that WRITES PRODUCTION DATA or FEEDS THE CANONICAL KEY — categorizers, capture, calibration applies, grading. Display fixes, docs, tests and read-only tooling get ONE REVIEW PASS. WHY: full adversarial cert is the most expensive thing this system does and it was being spent flat, which cost twice. It slowed the tier that needed it — cert capacity is one independent window and a serial bus, so every tier-2 subject in that queue is a tier-1 subject waiting (the #2161 categorization work took two adversarial rounds while the queue also carried subjects whose worst case was a wrong label on a page). And it cheapened the verdict: a BLOCK that can arrive over a stale figure in a report reads identically to a BLOCK over a categorizer that would mis-tag production rows, so the severity of the process stopped carrying information about the severity of the subject — under which reviewers drift toward pro-forma passes at the low-stakes end, exactly where the discipline then fails to be available at the high-stakes end. THE LINE is not 'is this important' (everything is) but: if this ships wrong, is the damage (a) written into data we will later read as truth, or (b) visible on a surface and fixable by the next deploy? (a) is tier 1, (b) is tier 2 — which is why WRITE and CANONICAL KEY are the named triggers rather than a directory list, since a wrong number on a page is a wrong number on a page while a wrong `is_winner` or `canonical_market_key` becomes the input to every later computation AND to every audit that would have caught it (ruling 130's shape: damage that survives the deploy that caused it is a different class). It is also why TESTS sit in tier 2 despite guarding tier-1 code — a bad test is loud, a bad categorizer is silent and its output is indistinguishable from a correct one without the corpus. BINDS: the proposing lane declares the tier in writing when it stages, before the review and not after a verdict it dislikes; the REVIEWER MAY ESCALATE AND THE AUTHOR MAY NOT DE-ESCALATE, and the re-tiering is itself the finding (the asymmetry is deliberate — the author is the party with an incentive to want the cheaper path); mixed diffs take the highest tier present, and splitting the commit to get the cheaper tier on the display half is encouraged while splitting it in the DESCRIPTION only is not; read-only tooling is tier 2 only while it stays read-only, and a write path resets the tier with no carry-over from its previous life; tier 2 is one pass by someone who DID NOT WRITE IT, a reduction in depth and never in independence, so author-never-certifies survives tiering intact; and tier 2 still records its pass in one line, because an unrecorded review and no review look identical (gotcha #53). DOES NOT lower the tier-1 bar (tiering exists so that process can be run properly where it matters), does not license shipping display bugs (one pass is a real gate and can send work back), and does not make 'read-only' a self-certifying claim — whether a change writes is a question about the code, and a lane that declares tier 2 on a path that turns out to write has mis-declared the thing the tier was computed FROM, which is the more serious finding. (Alex ruling, Q405 addendum 2, pasted and reviewed by Alex)
- [134](rulings/134-build-lanes-build-measurement-is-its-own-lane.md) — 2026-08-25 — BUILD LANES BUILD; MEASUREMENT IS ITS OWN LANE, AND AN IDLE BUILD LANE IS A SIGNAL: a build lane's only permitted measurement is its OWN GATES — tests, deploy checks, rollback verification. Every other measurement — censuses, probes, audits, diagnosis, and EVERY CERT — belongs to the measurement lane (the non-Claude windows on the mission bus), fed by `.claude/handoff/PARKED-MEASUREMENTS.md` and staged only when a NAMED SHIP needs the answer. Heavy measurement queries never run while an attended fold or apply is in flight. An idle build lane is a signal, not a failure — never fill it with measurement. WHY: a build lane holding a measurement capability will use it, and every individual use is defensible, which is the whole problem — nothing in the moment tells you that you have stopped building. The lane runs a census because the census would answer something, files what the census found, the finding earns a probe, the probe earns a re-measurement; each step is real work producing real artifacts and at the end of it the product has not changed. The failure is not laziness and it is not rigour: measurement is the PATH OF LEAST RESISTANCE for a lane that is blocked, waiting or between ships, because it always has something to offer, it never fails outright, and it produces an artifact that READS LIKE PROGRESS. So the fix is not 'measure less' but to make measurement something a build lane cannot reach for, by moving it to a different lane and requiring a named ship to pull it. THE BOUNDARY is not 'is this important' but WHOSE CORRECTNESS is being established: a gate answers 'did my change do what I said', a census answers 'what is the state of the world', and the second is a different lane's question even when the same lane is curious — the test where it is unclear is whether the answer would still matter IF THE CHANGE WERE REVERTED, and if yes it is world-state. The attended-apply clause is operational and has the sharpest edge: a heavy query landing beside a human-watched production mutation competes for the same database and turns a clean observation into an ambiguous one, and it does what ruling 132 forbids — churning the population you were about to measure. THE IDLE CLAUSE is stated as an instruction because it is the hardest to obey: a build lane with nothing to build is reporting a TRUE FACT (no named ship is ready for it), and filling it with measurement converts a legible signal ('we have no ship queued') into an illegible one ('everyone is busy') — an idle lane is cheap and visible, a lane busy on work that serves no ship is expensive and looks exactly like a lane serving one. BINDS: a build lane that finds itself running a census, probe, audit or diagnosis has ALREADY broken this and stops, parks the finding, and stages it with the ship named — discovering the breach mid-run is not a reason to finish the run; CERTS ARE MEASUREMENT, which reinforces author-never-certifies from the other direction (the author's lane is the wrong lane for the cert not only because it is the author but because it is a build lane); and an idle lane is never reported as a failure of the lane. DOES NOT devalue measurement or license building on unmeasured assumptions — the measurement lane exists, is staffed, and has a real queue, so this ROUTES the work rather than reducing it — and does not forbid a build lane from READING an existing measurement, which is what those artifacts are for. Nor does it make 'gate or census' a self-certifying judgment: a lane that labels a world-state query a gate to keep running it has mis-declared the thing the routing was computed from, which is the more serious finding (ruling 133's read-only-tier shape). General form: give the work that always has something to offer its own lane, and require a named ship to pull it — capability in the wrong lane is not capacity, it is drift with an artifact trail. (Alex ruling 2026-08-25, SHIP directive authored in Alex's Fable session and pasted by Alex who states he reviewed it; banked by lane1 Q406)
- [135](rulings/135-narrow-arm-a-to-the-live-slug-above-an-exposure-floor.md) — 2026-08-24 — A RELEASE NARROWS THE WINDOW; IT DOES NOT DISQUALIFY THE DAY — ABOVE A MINIMUM-EXPOSURE FLOOR. Partially amends ruling 130, which was right about attribution and turned out to be UNSCHEDULABLE, a different kind of wrong. Measured against the cadence it has to live on: 96 production releases in the twelve days 2026-08-12 -> 08-24 (95 intervals, median gap 0.67h, p90 11.03h, max 51.19h; gaps >= 24h = 1 of 95), so the number of UTC dates that could host a window with a deploy-free arm-A lookback was 2 of 12 at 24h with a LONGEST CONSECUTIVE RUN OF 2 — against #2107's requirement of SEVEN consecutive clean days. The falsifier was not strict, it was unrunnable, and an unrunnable falsifier grades INCONCLUSIVE forever, which reads to every future reader as "not yet proven" rather than "broken" — the `_detect_restart` failure again (#2107's own predicate was unconditionally true for eight days and nobody noticed), this time with a ruling behind it. THE RULING: arm A scopes its count to the live slug — `max(deploy_time, window_start)` forward instead of a flat 24h — and a day counts toward the seven only above a MINIMUM-EXPOSURE FLOOR, two floors, both failing closed to INCONCLUSIVE and never to CLEAN: exposure duration >= 6h since the live slug took over (below it the verdict stays STRADDLED, because "no failures in the 40 minutes since the deploy" is evidence about 40 minutes, not about the deploy), and served requests >= 50 in the window (six deploy-free hours in which nothing was asked of /api/feed is six hours of nothing observed; a bug that never had a chance to fire did not fail to fire). 6h is DERIVED, NOT CHOSEN — from the same 96 deploys, the qualifying-dates / longest-run table reads 24h: 2/2, 12h: 9/4, 6h: 12/12, 2-4h: 12/12 — so 6h is the most conservative floor that still admits a seven-day run, with five days of headroom, and 12h is not a stricter version of this criterion but an unrunnable one. THE RELAXATION AND THE SHARPENING ARE THE SAME EDIT, which is the clause worth keeping: an UNNARROWED count spanning two slugs cannot refute anything (the events may be the retired slug's) so its only honest verdict is INCONCLUSIVE, while a NARROWED count is attributable and a non-zero one is a genuine FAILED — scoping the interval is what turns arm A from a criterion that can only ever shrug into one that can refute the fix. Which is why narrowing is CHECKED rather than declared: a NARROWED verdict whose count was not in fact narrowed is refused, and so is one carrying no exposure number, since a claim about how much exposure it narrowed to is unfalsifiable without the number — both to INCONCLUSIVE. It does NOT touch arm B (500s on a single-slug window are that slug's whatever arm A is scoped to, so both floors and the narrowing sit BEHIND the 5xx check — a volume floor that swallowed a real refutation would be a hole, not a floor), and it does NOT credit unobserved time (without `--last-release-at` the takeover is bounded by the oldest recorded window that answered on the current slug and an observation gap is not filled in, because under-crediting costs a re-run tomorrow while over-crediting banks a day nobody watched). Implemented as the `NARROWED` verdict, `MIN_POST_RELEASE_EXPOSURE_HOURS`, `MIN_SERVED_REQUESTS` and `sum_buckets_since` in `backend/scripts/watch_2107_feed_500s.py`, pinned by `backend/tests/test_watch_2107_exposure_floor.py` (LAT-P088 addendum, Alex ruled Option B 2026-08-24; Fable directive pasted and reviewed by Alex)
- [136](rulings/136-the-falsifier-tests-a-code-change-not-a-slug.md) — 2026-08-26 — THE FALSIFIER TESTS A CODE CHANGE, NOT A SLUG: releases are TOLERATED inside a measurement window; what is excluded is the CUTOVER TRANSIENT, a named blast window after each deploy boundary. Amends ruling 135's arm A and retires ruling 130's straddle-disqualification for #2107. WHY: ruling 130 disqualified any window containing a deploy, ruling 135 narrowed arm A to the live slug above a 6h exposure floor, and BOTH were unrunnable — #2107 banked ZERO days in the three days ruling 135 was live. Measured 2026-08-27 over the 100 most recent releases (2026-08-14T16:59:38Z -> 2026-08-27T00:45:58Z, 295.8h, 0.34 releases/hour, median gap 0.63h, p75 1.38h, p90 11.64h, gaps >= 6h = 11 of 99): only 52.1 % of wall-clock has >= 6h since the last release and 22 % of random 60-minute windows contain one, so a day banked at ~41 % per attempt — and worse in practice, because without `--last-release-at` the 6h bound came from walking recorded windows back until the SHA changed and at one window per day consecutive windows essentially never share a SHA, so arm A graded STRADDLED every time. An INCONCLUSIVE day is NOT neutral: `summarize` requires calendar-consecutive clean dates, so a day that cannot bank is a gap and a gap resets the streak exactly as a failure does — at p = 0.41 the expected wait for seven consecutive banked days is ~868 days. THE MISTAKE UNDERNEATH ALL THREE is that ruling 130 attributed to the wrong object: a window spanning a release measures two different SLUGS, but the thing under test is a CODE CHANGE (`b2e3e1a9` + `42f2356b`) present in every slug deployed since it merged, so a boundary between two fix-carrying slugs is not a change of the system under test — only the cutover transient is. THE RULING, five pre-registered clauses: (1) releases are tolerated and `MIN_POST_RELEASE_EXPOSURE_HOURS` is retired; (2) `DEPLOY_BLAST_WINDOW_MINUTES = 10` — an error inside it can neither pass nor fail the window and grades INCONCLUSIVE, logged with the boundary it was measured against; (3) errors OUTSIDE the blast window are FAILED whether or not a release landed, which is the sharpening half, since ruling 130 converted every such refutation into a shrug; (4) the exposure floor is counted in REQUESTS not hours — `MIN_SERVED_REQUESTS = 50` served outside any blast band, because a floor in hours measures the deploy cadence while a floor in requests measures the exposure; (5) EVERY OBSERVED SLUG MUST CONTAIN THE FIX, checked with `git merge-base --is-ancestor` against every commit `/api/health` reported, because tolerating releases without it would let a ROLLBACK to a pre-fix slug bank a clean day for a fix that was not running. 10 MINUTES IS DERIVED, NOT CHOSEN: all 35 BAINLUCK-ZK events were timestamped against the preceding release, giving wall-clock-covered / events-inside / enrichment of 2min 1.1%/0/0.00x, 5min 2.7%/8.6%/3.12x, 10min 5.4%/11.4%/2.12x, 15min 7.9%/14.3%/1.80x, 20min 10.3%/14.3%/1.38x, 30min 14.4%/22.9%/1.59x, 60min 22.3%/31.4%/1.41x — enrichment peaks at 5min and is back to background by 20min, the signature of a transient that dies out in minutes; the four near-deploy events sit at 3.1, 3.3, 4.7 and 7.1 minutes so 10 covers all of them at 2.12x while costing 11.4 % of detection, and the bug is overwhelmingly NOT a deploy artifact (median event fires 517 minutes after the last release). SHORTER IS FAIL-CLOSED — a short B grades a transient FAILED (costs a re-run), a long B grades a real regression INCONCLUSIVE (costs the falsifier) — so 10 is an UPPER BOUND justified by the enrichment curve and 30/60 are rejected despite being more tolerant. The default window moves to 90 minutes for the same reason the floor is in requests: a blast band costs ~10 samples, and simulated across the whole span (candidate start every 5 minutes) the share of start times still clearing 50 served requests reads 60min 3184/3538 = 90.0 %, 75min 97.0 %, 90min 3524/3532 = 99.8 %, 120min 100.0 % — 60 leaves a single mid-window deploy sitting ON the floor at 49, which is the ruling-135 mistake in miniature, and expected wait to seven consecutive clean days falls from ~868 days to ~7.1. DOES NOT weaken arm B into 'errors near a deploy are fine' (a blast-window error is recorded, printed, and blocks the day from banking — it buys attribution, not silence), does NOT retire ruling 130's general form (where the subject genuinely IS a slug — a rollout, a config flip, an infrastructure change — 130 stands unamended; 136's claim is narrower: for a falsifier whose subject is a code change present in every deployed slug, a slug boundary is not a change of system), does NOT touch the RESTART arm (a restart clears the process-globals under watch, so it IS a change of the system under test — the contrast is what makes the amendment coherent), and does NOT lower the bar to closure (seven consecutive clean UTC dates on both arms is unchanged; what changed is that seven is now reachable). Two defects surfaced only because the release rule stopped disqualifying every window before they were reached: `SENTRY_ORG` defaulted to `bain-luck` which 404s (the org is `alexander-bain`), so arm A was one unset env var from permanently UNKNOWN; and the single window ever recorded carried `is_day: false` and could never have banked. Implemented as `DEPLOY_BLAST_WINDOW_MINUTES`, `deploy_boundaries`, `attribute_errors`, `check_fix_ancestry`, `sentry_events_since` and `streak_from_rows` in `backend/scripts/watch_2107_feed_500s.py`, pinned by `backend/tests/test_watch_2107_blast_window.py` (82 pass; proven RED at 67/68 against the retired criterion); pre-registration record `docs/audits/latency/lat-p098-2107-closure-gate-respec.md` (Alex directive authored in Alex's Fable session 2026-08-26, delivered through the lane runner Alex launched under his standing authorization)
- [137](rulings/137-the-headline-is-the-cold-path-a-user-walks.md) — 2026-08-26 — THE HEADLINE IS THE COLD PATH A USER WALKS, NOT THE WARM ONE THE CACHE CAN PRODUCE: amends ruling 127 §1 and `docs/PRD.md`'s latency charter. Alex's words: "stop bragging about warm searches — a tiny fraction of searches will be warm. What matters most: Discover load time, the load time of the other tabs, and COLD search load. That's what a user experiences in volume." WHY: ruling 127 had already banked the sharper half — a p50 over mixed cache states is a statement about the HIT RATE, not about latency — and then made the warm number the headline anyway, which is the defect, and it is worth naming precisely because 127 was RIGHT: the fault was not a missing insight but an insight applied to the footnote and not to the headline. `feed p50` opened three consecutive reports at 18ms, 17.5ms and 20.7ms, every one true and correctly measured, while the miss underneath cost 3,201.7ms (LAT-P097) and 4,100ms (the PRD's first honest measurement) — THE HEADLINE AND THE FINDING DISAGREED FOR THREE CYCLES AND THE HEADLINE WON EVERY TIME, which is what a headline is for and why choosing one is a ruling and not a formatting preference. THE NEW SET, opening every report from LAT-P099: Discover first-load p50, Sports first-load p50, Browse first-load p50, My Stuff first-load p50, and cold search p50 (`/api/events/search` + `/api/events/typeahead`, first touch, never-asked term) — five rows because the native tab bar has FIVE tabs (`Views/MainTabView.swift:19-52`), Search being both the fifth tab and the surface the cold-search row grades. WARM IS DEMOTED, NOT DELETED: `done_bar_snapshot.py` keeps measuring warm feed/typeahead and keeps their series (a series re-baselined mid-flight is worth nothing — this program has paid for that twice, in the salt and in the term set), but a warm-hit win may NEVER open a report or lead a claim again. THREE DEFINITIONS make it enforceable: a first load is the request a tab issues on an install the server has never served, sampled with a FRESH `x-session-id` per sample because the feed key is per-principal — repeatable precisely because the LAT-P089 inert-principal share republishes only to the PRIVATE key, so a fresh-session sample can READ what the warmer left and can never EXTEND it; a TAB IS A REQUEST SET and only one member gates the paint (Sports issues three, My Stuff two, Discover two — summing overstates the wait, reporting only the main one hides a sibling that can hang), so the `blocking` flag is taken from the client's own control flow and the rest is printed beneath; and a surface that issues NO request is reported as NO SERVER DEPENDENCY, never as "0 ms" (Browse renders static arrays, `Views/LeaguesView.swift:55-78`; the web Browse is a link dropdown with no route) — a request never issued has no latency, and printing a zero as though an instrument produced it is the same class of error as a gate that cannot fire, because it reads as a measured pass. THE BARS ARE INHERITED AND A CYCLE MAY NOT MOVE THE ONE IT IS ABOUT TO FAIL: tab first-load p50 <= 1,000ms taken verbatim from the charter's existing feed-miss bar, cold search p50 <= 1,000ms, cold typeahead p50 <= 500ms unchanged, plus a per-sample HARD CEILING of 6,000ms = `DiscoverViewModel.retryBudget` (:216), a non-retryable client deadline past which the app gives up and paints disk last-good — graded on the MAX and not the median, because one sample over it is a user-visible failure a median hides. DOES NOT loosen any instrument clause of 127 (organic-first census read, observer subtraction, contamination declaration, derived transport-floor threshold, frozen term set all carry over, and the new instrument enforces the first MECHANICALLY via `--stats-before` rather than by memory), does NOT make cold-only the rule (the headline is the USER-VOLUME number over the real cache mix with the cold sub-median beside it — a tab warm 90% of the time genuinely is fast 90% of the time, and pretending otherwise is the same error sign-flipped), does NOT retire `done_bar_snapshot.py` and does NOT license subtracting its numbers from the new instrument's (127: a delta between two different measurements is a delta of instruments), and does NOT touch the reliability bar or cert tiering. General form: WHEN A SYSTEM IS BIMODAL THE HEADLINE MUST BE WEIGHTED BY THE MODE USERS ARE ACTUALLY IN — otherwise the metric improves every time the cache gets luckier and never when the product gets faster. Implemented as `backend/scripts/cold_path_snapshot.py`; pre-registration record `docs/audits/latency/lat-p099-cold-path-charter.md`, committed before the first number was taken (Alex directive authored in Alex's Fable session 2026-08-26, delivered through the lane runner Alex launched under his standing authorization)
- [138](rulings/138-price-is-not-a-word-we-say-to-readers.md) — 2026-08-27 — "PRICE" IS NOT A WORD WE SAY TO READERS; THE WORD IS PROBABILITY. Alex's words: "'price' as a noun is banned in user-facing copy — the word is PROBABILITY." Permanent and product-wide, and it REPLACES the line UX-P145 drew the same morning, which banned *priced* as a verb ("nobody has priced it yet", "a priced round to reach") while KEEPING *price* as the noun a market publishes, on the reasoning that the boards, the slate and `/calibration` already shared that vocabulary and splitting them would make three surfaces word one admission three ways. WHY THAT WAS WRONG: the reasoning optimised consistency, and consistency was the wrong target. The product thesis is the first line of CLAUDE.md — we translate markets into intuitive probabilities so a reader sees 60% vs 40% instead of -150/+130 — so the entire value we add is that nobody has to think in the trading layer, and a page that does that arithmetic and then calls the result a *price* hands the trading layer back in the caption, after having removed it from the number. Three surfaces consistently using the wrong word is not an argument for keeping it; it is three places to fix. GENERAL FORM: when a product exists to translate a domain's vocabulary, the translated surface may not carry the source vocabulary IN ITS OWN VOICE — least of all in the sentence that explains the translated number. DOES NOT TOUCH data contracts (`price_state`, `data-price-state`, `opening_probability`, `priced_cells` all keep their names — CERT-411 and the sentinels read them, no user sees them, and the sweep runs over rendered TEXT with attributes stripped so it is indifferent by construction rather than by an exception list), does not touch code or comments, and does not touch the English word where the subject really is a price (`/economics` keeps "Inflation & Consumer Prices" and "the price at the pump" — those are prices of goods in the world, which is what those markets are about). AND IT DOES NOT COST THE HONESTY THE OLD COPY CARRIED: "Prices paused… these are the last prices we saw, not live prices" became "Updates paused… these are the last probabilities we saw, not live ones", identical in force and specificity, and the guard pins BOTH halves — the banned word absent AND the staleness still stated — because the failure mode of a copy ruling is a rewrite that removes the word and the meaning with it. SWEPT by UX-P146 on the tournament surfaces (13 strings, verified at the render, including the ones authored in `backend/app/utils/tournament_grid.py` that reach readers as cell tooltips and screen-reader text); OWED and named in the ruling rather than counted as done: `/calibration` (the heaviest user, and the one needing care rather than find-and-replace — the `price_moved` dimension is a real distinction about TRADING and must keep meaning what it meant), `/futures/[id]` + `FuturesChart` + `lib/priceCadenceCopy.ts`, `lib/propDivergence.ts`, and `/admin/matching`. GUARD: `tournamentPlainLanguage.test.tsx` bans the whole stem `/\b(un)?pric(e|es|ed|ing)\b/i` over rendered text on every tournament surface, replacing UX-P145's eleven hand-written variants, with a canary that renders each retired sentence and requires the sweep to reject it, plus a source scan of `components/tournament/*.tsx` JSX text nodes for the states no fixture reaches (Alex directive authored in Alex's Fable session 2026-08-27, delivered through the lane runner Alex launched under his standing authorization)
- [139](rulings/139-a-near-duplicate-rule-may-never-collapse-across-subjects.md) — 2026-08-27 — A NEAR-DUPLICATE RULE MAY NEVER COLLAPSE ACROSS SUBJECTS. Alex's words: "alcaraz-second-major and sinner-second-major are DIFFERENT PLAYERS and must both render. Key the near-duplicate rule so it never collapses across players." AMENDS the READING of UX-P138 ruling 8 ("never a repeating template"), not its clause. The implementation read *template* as "the same question with a different name in it": `propTemplateFamily` dropped the leading token of a register key, so `alcaraz-second-major` and `sinner-second-major` both reduced to `second-major`, one card was kept and the other counted as `dropped.template` — and a later hand-edit did the same thing at the source, moving `alcaraz-second-major` into the register's `props_declined` as "one question with two names in it". WHY THAT WAS WRONG: two rivals' odds of the same feat is not one question asked twice, it is THE COMPARISON, and on a two-horse men's draw it is the most interesting thing the section has — measured on Kalshi 2026-08-28T00:5xZ, Alcaraz's `2+ Grand Slam wins` is 27c on 42,723 open interest and Sinner's is 1c because his "to play" market is also 1c, which side by side IS the state of the men's draw and separately is trivia. The cap was not too strong, it was KEYED WRONG — on the topic instead of the topic AND the subject — so it could not tell "we are repeating ourselves" from "we are drawing a contrast". GENERAL FORM: a rule that suppresses an item for resembling another must key on everything that makes the two items answer different questions; dropping a field from the key to make the rule fire more often is not a stronger rule, it is a rule that has stopped being able to see the distinction it exists to protect. The failure is silent by construction — a suppression rule reports a COUNT, so the section looks curated rather than lossy and the deleted item leaves no trace on the page — which is why the ruling constrains the KEY rather than a threshold. REACHES Discover story caps and semantic dismiss (gotcha #25, whose generic-token exclusion is this clause already), cross-source near-match's numeric and direction guards, and any future one-card-per-family rule (the test is not "how similar are these" but "if a reader saw both, would they learn something from the second"). DOES NOT CHANGE: the cap still exists (one card per WHOLE register key, same subject + same topic still collapses, drop still counted), nothing is inferred from the title so the rule cannot be defeated by rewording, and ruling 3's advance-to-round routing is untouched — with one consequence worth writing down, that a curated question ABOUT a round which is not a per-player advance market ("how many Americans reach the quarter-finals") must not be keyed `*-quarterfinals`, or `advanceRound` routes it to a grid that has one row per player and no row for a count. GUARD: `tournamentPickerRotation.test.tsx` ("ruling 8 as amended") asserts the family key, that the two second-major cards do not merge, that BOTH RENDER through the shipped component, and that the same question about the same subject still collapses — the render assertion being the load-bearing one (Alex directive authored in Alex's Fable session 2026-08-27, delivered through the lane runner Alex launched under his standing authorization)
- [140](rulings/140-an-inference-needs-a-population-that-can-refute-it.md) — 2026-08-28 — AN INFERENCE MAY REACH A USER SURFACE ONLY WHERE AN INDEPENDENTLY-PINNED POPULATION CAN REPLAY IT AS A TEST. Banked by the program-ux lane from UX-P149 and FLAGGED TO ALEX FOR RATIFICATION, not self-certified. **RATIFIED PROVISIONALLY BY ALEX 2026-08-28**, in his words: "happy to ratify and try... we can revert it if it's causing problems." PROVISIONAL IS A REAL STATE: the clause is in force now (a lane may cite it as settled doctrine, subject to its three obligations) and is revertible ON EVIDENCE rather than on argument — the thing that takes it back is a measured problem (a mis-attributed card, a refusal rate that empties a page, a replay population that stops refuting), never a later lane's better reading of the same case, so a lane wanting 140 loosened owes a SPECIMEN and not a paragraph. WHAT A REVERT WOULD COST, stated so the option stays real: the licence is load-bearing for exactly ONE shipped surface (`MatchProps` on the event page, via `attribute_yes_side`), so reverting means those cards print `Yes 53%` or nothing — that containment is why provisional is affordable here and would not be for a clause with ten call sites. THE RE-READ TRIGGERS are each already an executing test — a real mis-attribution in production, the replay dropping below 28/28 on the register's pins, or the refusal rate rising off `0 of 205` far enough that the guard is emptying the page rather than the data — because a provisional ruling whose revert condition is unmeasured is a ratified one with an apology attached. THE CASE: Polymarket stores a match prop's two outcomes as the words `Yes` and `No`, and which player is `Yes` is stored NOWHERE — `futures_outcomes.name` is the word, the external id is `…_yes`, and the label the source printed is dropped at ingestion. The register pins `sides` offline for the match-WINNER market, which is why the slate prints two players instead of Yes and No; it cannot do that for ~12 props × 96 matches, which is lane1's own reason for keeping them off the register. So the page either infers the attribution or ships without its most legible half ("Who wins set 1" is the question closest to the one the reader came for, and `Yes 53%` is not an answer to it). THE RULING: an inference is admissible only where an INDEPENDENTLY-PINNED POPULATION exists over which the same rule can be replayed as an executing test. Here that is the register's 28 live matchups, whose mapping was established offline from the source's ordered labels by a different process on a different day — so it is not this rule's own output but a second and better source for the same fact. The rule (`_yes` is the player named FIRST in that market's own title) holds 28 OF 28, ZERO VIOLATIONS, and `test_the_yes_side_rule_holds_on_every_register_pin` replays it against a committed capture of the real titles, so the day the source flips its convention the register's own pins turn red rather than the page printing a number under the wrong name. WHAT IT FORBIDS: an inference with no such population — one whose only evidence is that it looked right on the rows somebody eyeballed. The distinction is not confidence, it is FALSIFIABILITY BY DATA WE ALREADY HOLD. A SOFT CORROBORATION IS NOT A SUBSTITUTE and is recorded as a number, not a feeling: opening-vs-opening across the prop class gives 51 agree / 2 disagree (both marginal), mean gap −0.16 in favour — and its FIRST run, on current prices, reported 10 disagreements that were every one a settled match-winner market compared against an unsettled prop, which is the second reason a soft check may not be load-bearing (its own failure modes are less well understood than the thing it is checking). THREE OBLIGATIONS COME WITH THE LICENCE: (1) REFUSE rather than guess — both players present as whole words, at distinct positions, on tokens not shared between them, and a handicap's minus on the first-named side; measured 0 of 205 real titles refused, asserted so a future tightening shows its cost rather than quietly emptying the page; (2) COUNT every refusal into the payload (`props_dropped`), because a surface that silently omits what it could not attribute reads as complete; (3) READ THE ITEM'S OWN EVIDENCE, never a sibling's — 5 of 73 real prop titles name the players in the OPPOSITE order to their own match-winner market (all Set Handicaps, where the source puts the favoured side first), so inheriting that order would have mis-attributed five cards that would each have looked entirely plausible. WHY THE BAR IS HERE: a wrong number under a real player's name is not visibly wrong, is not self-correcting, and the reader cannot detect it — and the inference buys only the minority of the page, since 128 of the 206 prop markets measured are Over/Unders needing no attribution at all. SITS BESIDE ruling 048, which draws this exact line for ABSORPTION; this is the same line drawn for DISPLAY. REACHES `prediction_market_matching.py`, `cross_source_matching.py` and any future surface deriving a player, team or side from a market title (program-ux lane, UX-P149)
- [141](rulings/141-venue-names-are-not-part-of-the-product-a-reader-bought.md) — 2026-08-28 — VENUE NAMES ARE NOT PART OF THE PRODUCT A READER BOUGHT. Alex's words: "venue names — Kalshi and Polymarket — are BANNED in user-facing copy, everywhere. Users get our probability, not our sourcing." AMENDED BY ALEX THE SAME DAY, and the amendment is the operative text: "the venue-name ban was overinterpreted. The precise rule: venue names are BANNED in narrative/empty-state/promotional copy (the US Open empty-state class — talking ABOUT sources instead of showing the product), but ALLOWED — and often good — as SOURCE ATTRIBUTION of a number or line the user is looking at" (trend-graph line labels, the calibration page naming each source, source chips beside a figure). Permanent and product-wide, issued from the same live-page reading as rulings 138 and 142. This is ruling 138's clause one level up: 138 banned the trading layer's noun, this bans its proper nouns. A reader shown one blended number and then told which two exchanges it came from has been handed the plumbing back after we removed it, and handed a decision the product exists to make for them — the standing "the blend is the product" ruling, applied to the caption. THE SHARPER REASON: naming the venue makes an absence sound like a supplier problem. The grid legend said "we asked Kalshi and Polymarket and neither runs that market", which tells a reader we have two suppliers and both let us down; what is true is that NOBODY is answering that question anywhere, which is both shorter and more useful. GENERAL FORM (as amended): a translated surface may not SELL the sources it translated — where the abstraction is the product, what sits under it is implementation detail WHEN IT IS OFFERED AS THE SUBJECT, and putting it in front of a reader in that form converts a finished answer back into a research task. Naming a source as the ATTRIBUTION of a number the reader is already looking at is not selling it. DOES NOT TOUCH source ids (`source: "kalshi"`, `group_id: "polymarket:12345"`, `stale_sources`, every enum and data attribute — the guard is CASE-SENSITIVE precisely so the lowercase ids pass and the capitalised names do not), code, comments, `/admin` (an operator needs the exact venue and ticker), `/privacy` (a legal disclosure must name who we read), or deliberate comparison surfaces (`/calibration` publishes how well each SOURCE predicts; a source-accuracy table with the sources anonymised is not a stronger version of itself — first LISTED as debt because that was Alex's call and not a sweep's, and the amendment makes the call: EXEMPT). THE HONESTY SURVIVED BOTH REMOVALS, pinned in both directions: the no-market legend keeps "the question is unanswered, not unread", and the muted-row line went from "Polymarket 20 days ago" to "one reading 20 days ago" — the venue name never carried that admission, the PARTITIONING did, and a count says "only part of this is old" identically, in the page's own existing "no reading yet" vocabulary. SWEPT by UX-P150 on the tournament surfaces under the UNAMENDED clause, and RE-SCOPED by UX-P152: the guard now judges attribution vs narrative per occurrence (`isSourceAttribution` in `lib/copyBans.ts`) instead of matching the name, `/calibration` and `/privacy` moved from debt to EXEMPT, `/politics` and `/weather`'s source chips turned out to be attribution and were deleted from the debt list rather than fixed, and the `CombinedFeedCard`/`SourceComparisonRow` legends that were called "the next one that should go" are allowed — the call reverses. No legitimate attribution label had been stripped: UX-P150's four venue removals were all narrative or empty-state, and `sourceLabel` had exactly one caller. STILL OWED, measured against the built bundle and recorded as executable debt in the `OWED` map (keyed on surface AND rule, so an unlisted surface fails and a new kind of violation on a listed surface also fails): four narrative sentences — `/about`'s "Kalshi + Polymarket, unified", `/categories/golf`'s "Tournament odds from Polymarket, Kalshi, sportsbooks & DataGolf", `/weather`'s "Daily Will it rain? markets from Kalshi", and the shared landing blurb "…each have a guess". `app/layout.tsx`'s meta keywords are not visible to a reader, so out of scope on this ruling's own terms, but flagged to Alex as an SEO call rather than a copy one (Alex directive authored in Alex's Fable session 2026-08-28, delivered through the lane runner Alex launched under his standing authorization)
- [142](rulings/142-a-section-states-what-it-is-not-what-it-will-be.md) — 2026-08-28 — A SECTION STATES WHAT IT IS, NOT WHAT IT WILL BE; AND A COPY GUARD READS WHAT SHIPPED. Alex's words: "no future-tense promises in product copy — we shouldn't be talking about what the section 'will be', we should make it the thing it's supposed to be." THE SENTENCE: the live props empty state read "Once the main draw starts, Kalshi and Polymarket list more of them beyond who-reaches-what, and the ones worth asking appear here as they are priced" — venue names (141), *priced* (138), and its whole length spent describing a section that does not exist yet. WHY A PROMISE IS WORSE THAN AN ADMISSION: we do not decide when a market opens a question, so "as soon as they have a number" has no date behind it and is reassurance rather than information; it mis-states the failure (the reader hears "waiting on a supplier" when the truth is the narrower and more useful "nobody is answering that question right now"); and it decays into a lie — with CERT-422 the section HAS content, and a promise that outlives its delivery is worse than the apology it replaced. GENERAL FORM: a surface describes its present state, not its intended one; where a product cannot name a date it may not name a time. DOES NOT BAN the bare auxiliary — half the questions on the page are questions a MARKET wrote ("Will Sinner actually play?"), and a rule broad enough to catch those gets switched off within a week, so every pattern is a phrase only our own voice produces ("check back", "coming soon", "appear here", "as soon as anyone", "once the main draw starts") and the market questions are carried as explicit ALLOWED cases. SECOND HALF, and the load-bearing one: A COPY GUARD READS WHAT SHIPPED. Every string Alex quoted had ALREADY BEEN FIXED — "No prices yet" and "Prices paused" by UX-P146, "Nothing curated yet" and "gone dark and rotated out" by UX-P145 — and none had landed. Measured the same day against the chunks bainluck.com actually serves: 41 VIOLATIONS ACROSS 30 DISTINCT SENTENCES, including the entire output of both sweeps. The guards were not weak, they were pointed at the wrong object: source files and `renderToStaticMarkup` fixtures, neither of which is a reader. GENERAL FORM: a guard proves a property of the artifact it reads, so when the claim is "a user no longer sees X" the guard must read something a user could have downloaded — and where it cannot, it must say so out loud rather than pass. BUILT: `lib/copyBans.ts` (rules for 138/141/142 + the UX-P145 jargon list as data, with a minified-JS string extractor), `shippedCopyBans.test.ts` (predicate pinned against every retired sentence AND the market questions it must not eat; the local `.next` build; and production via `SHIPPED_BUNDLE_DIR`), `scripts/fetch-shipped-copy.mjs` (carries no rules, so it cannot drift from them). THE RULE THAT FOLLOWS: a copy ruling is not closed by a green branch — it is closed by `SHIPPED_BUNDLE_DIR=… npx jest shippedCopyBans` reading zero on the surface it names, against a bundle fetched from production after the deploy (Alex directive authored in Alex's Fable session 2026-08-28, delivered through the lane runner Alex launched under his standing authorization)
- [143](rulings/143-two-rivals-one-question-is-one-card.md) — 2026-08-28 — TWO RIVALS ASKED ONE QUESTION ARE ONE CARD, WITH BOTH NUMBERS ON IT. Alex's words, MC 2026-08-28 ~10:45am PT: "ONE COMBINED CARD — 'Who wins a second major this year?' — showing BOTH players' probabilities (Alcaraz 2+ majors, Sinner 2+ majors, each from its own real Kalshi market: KXGRANDSLAM-CALC26-family and KXGRANDSLAM-JSIN26)." THE CASE, and why it took three queues: the register carried `alcaraz-second-major` (.25) and `sinner-second-major` (.555) as two cards asking one question. UX-P138's note called them "one templated question with the name swapped" and what shipped for it DELETED ALCARAZ — at render time through a topic-keyed near-duplicate cap, then in the file by hand. UX-P147 overruled that ("DIFFERENT PLAYERS and must both render", "I'd love to" see both), restored him, and rekeyed the cap on the whole register key (ruling 139) — bringing the repetition back with him. Both notes were right and both readings were partial, because both assumed the unit is ONE CARD PER MARKET; drop that and one card carrying both men's numbers satisfies each note exactly. THE RULING: where two curated markets answer the same question about different subjects, the REGISTER may compose them into one card whose rows are the subjects. FOUR GUARDS COME WITH IT: (1) a combined card names NO answer — two legs means no single outcome answers the question, so `answer_entity_key` is null and the renderer ranks instead of guessing a headline (the field shape `validate_prop` has supported since UX-P134, first curated use); (2) a MISSING LEG REFUSES THE WHOLE WRITE — a comparison card with one side is not a smaller card, it is "who wins a second major" with one man under it, which reads as an answer and is not one, and it is the defect both earlier queues shipped; (3) the rows are RENAMED from the source's `2+ Grand Slam wins` to the men's names, with `evidence.legs[].source_outcome_name → renamed_to` recording the rename, same doctrine as the matchup `sides` mapping; (4) the legs are NOT NORMALISED — four majors a year and each man needs two, not the same two, so both can resolve Yes and the numbers do not sum to 100; because the title reads as a race the hook is load-bearing ("These are two separate questions — they could both do it, or neither"). DOES NOT WEAKEN RULING 139: a register composing two named markets offline is curation; a render-time cap picking which of two curated cards a reader gets is a silent deletion, and 139's guards are kept with synthetic keys because the clause outlives its case (081). SHIPPING TRUTH: both Kalshi markets were last read 2026-07-24, so on merge the card is 856h old and rotates out — what ships is the SHAPE, and the card appears when the registered-market refresh arm (window 424, unmerged) reaches them (Alex ruling relayed through the UX-P151 runner directive, executed by the program-ux lane)
- [144](rulings/144-no-ruling-is-attributed-to-alex-without-his-words.md) — 2026-08-28 — NO LANE ATTRIBUTES A RULING TO ALEX WITHOUT QUOTING HIS WORDS. Alex's words, with the UX-P151 directive: "no lane may attribute a ruling to Alex without quoting his actual words with date and source (chat/MC). 'Exactly as ruled' without a quote is a violation — this queue exists because a paraphrase inverted a ruling. Fable audits every 'as ruled' claim that reaches YOUR-TURN." THE CASE: ruling 143 exists because a lane recorded "one templated question with the name swapped" as a ruling to remove the repetition and removed a PLAYER, costing three queues to undo. THE MECHANISM, named because it is not carelessness: a paraphrase is a lane's INTERPRETATION wearing Alex's authority — once it is in a comment or a test name ("// Alex's item 11") it is indistinguishable from the ruling, cannot be checked against anything, and every downstream lane inherits it as settled. A quote can be wrong about its own meaning and still be re-read; a paraphrase cannot be re-read at all, because the words it replaced are gone. REQUIRED: the verbatim sentence, the DATE (rulings are amended; an undated quote cannot be ordered against its own amendments), and the SOURCE (chat/MC/review/directive — a lane reading a directive is reading a relay and must say so). "Exactly as ruled" with no quote is a violation, and so is a bare "Alex's item N", which is an index into a document the next reader does not have. WHERE A LANE MUST ACT ON ITS OWN READING of an ambiguous note it says so in those terms — *this lane read X as Y* — so the reading is falsifiable rather than authoritative. TWO OBLIGATIONS: the directive's AUTHOR carries Alex's words into it; the EXECUTOR carries them into the code, the guards and the report without upgrading its own reading into a quote on the way. DIRECTIVE-AUTHORING GUIDANCE LIVES IN THIS FILE. GENERAL FORM: an attribution is a claim about evidence and takes the same bar as any other — this is ruling 140's clause applied to authority instead of to data, where the refuting object for "Alex ruled X" is the sentence he wrote (Alex standing doctrine addition, relayed through the UX-P151 runner directive)
- [145](rulings/145-a-template-family-is-one-card-and-the-system-finds-it.md) — 2026-08-28 — A TEMPLATE FAMILY RENDERS AS ONE COMBINED CARD, AND THE SYSTEM FINDS THE FAMILY. Alex's words, reviewing UX-P151: "clearly looks better" — and then "BUT: Was this a bespoke solution? I thought we'd built tools to identify groups and surface them as groups. Why didn't any of them trigger?", with the instruction "GENERALIZE: template-family props render as one combined card BY THE SYSTEM. FORMATTING pillar: bespoke solutions to systemic shapes are defects." WHY NOTHING TRIGGERED, answered from the code in three measured parts: (1) the props pipeline's one family concept was a CAP, not a grouper — `curatedProps` asked `propTemplateFamily` for a key and on a repeat did `dropped.template += 1; continue`, so the ONLY TWO THINGS THE MACHINERY COULD EMIT were two cards or one card and a deletion, which is exactly why UX-P138 got the deletion and UX-P147 got the repetition and neither had a third option; (2) that cap had been STRUCTURALLY UNREACHABLE since UX-P147 keyed it on the whole register key, because register keys are unique by construction — `dropped.template` could not be non-zero, so the rule everybody was reasoning about was dead; (3) the real grouper EXISTS and was pointed elsewhere — `prop_families.py::group_prop_families` does exactly what Alex remembers but its only consumer is `GET /api/teams/{...}/prop-families`, and it would not have fired anyway since `family_key("Carlos Alcaraz: Grand Slam wins in 2026")` returns None (its vocabulary is Next Team / of-the-year / awards / "<entity> to <verb> N <unit>" / over-unders, and this is a season-total ladder with the subject before a colon). `market_grouping.py` groups on a provider's own event id, a different axis, and the two Kalshi series share none. SO THE DETECTOR WAS BLIND TO THE SHAPE AND THE RENDERER HAD NO COMBINED OUTPUT — both halves missing, which is why a human wrote the legs. THE RULING: where two or more curated markets ask one question about different subjects they render as ONE card with one row per subject, and the SYSTEM decides that. TWO LAYERS, both required: `app/utils/prop_template_family.py` detects families among the MARKETS (two markets are one question about different subjects when their titles differ in ONE CONTIGUOUS RUN of tokens and they SHARE AT LEAST ONE OUTCOME NAME) so the register is written composed; and `combinePropFamilies` merges same-topic/different-subject cards at RENDER so the guarantee holds for every register already committed. FOUR PROPERTIES: it combines or it renders BOTH, never deletes (ruling 139 satisfied in substance — where the members' titles cannot name a combined question the cards render separately, because repetition a person can see beats a card they cannot); the WORDS stay curated and the COMPOSITION stops being, with the pass REFUSING a detected family nobody wrote a question for and refusing a curated family it cannot find; row labels are the SOURCE'S own words ("Carlos Alcaraz", not a hand-written "Alcaraz") per Alex's item 4; and the test of "by the system" is A THIRD SUBJECT joining with no code change. TWO BARS with the specimen that set each: `MIN_SHARED_TOKENS = 2` because a real family can have a short skeleton ("LeBron James Next Team"), and `MAX_SUBJECT_RATIO = 1.0` because "Alcaraz to win the US Open in 2026" vs "Sinner to win the Australian Open in 2026" is one contiguous difference over three shared trailing tokens and TWO DIFFERENT TOURNAMENTS — the difference had swallowed the question, and the first run of this rule paired them. AND THE CONDITION THE REAL DATA REFUSED: the outcome half started as "the same SET of outcomes", which reads better and is wrong — measured 2026-08-28, Alcaraz's ladder lists `2+ / 3+ / All 4` and Sinner's lists `1+ / 2+ / 3+`, so an identical-set rule would have FAILED TO DETECT THE ONE FAMILY WE ALREADY KNOW EXISTS; the family carries the INTERSECTION and the compared outcome must come out of it. GENERAL FORM: a systemic shape gets a systemic solution, and the test of whether one is systemic is not whether the output is right today but whether it is still right after the input changes in the ordinary way inputs change (Alex ruling relayed through the UX-P154 runner directive, executed by the program-ux lane)
- [146](rulings/146-an-illiquid-question-is-still-a-question.md) — 2026-08-28 — AN ILLIQUID QUESTION IS STILL A QUESTION: A PROP IS NEVER HIDDEN FOR AGE, AND THE AGE SAYS WHAT IT IS THE AGE OF. Alex's words: "NEVER EXCLUDE PROPS — illiquid props render with honest freshness indication, never hidden, that's part of the value of the product", and on the treatment: "staleness is per card, not per section, because liquidity varies within a section", "the '32 hours ago' ambiguity is real — created? updated? last traded? Define what the timestamp MEANS, label it so a reader knows", "continuing to riff on this until we have a better solution would be great", "this is an open riff, not a settled design". OVERRULES the second half of UX-P138's ruling 8 ("when a prop resolves or goes stale, it rotates out"); the first half — an advance-to-round question is a grid cell, not a prop — stands. THE CASE: `More predictions` on `/tournaments/us-open` has been EMPTY ON PRODUCTION EVERY DAY SINCE IT WAS BUILT. The register held three curated questions, all three older than the 48-hour boundary, all three dropped — and FOUR SEPARATE QUEUES then improved the wording of the apology that appeared in their place (UX-P139 made the empty box visible, UX-P145 rewrote it into the reader's vocabulary, UX-P150 trimmed the venue names, ruling 142 removed the promise at the end). Every one was the right fix to the wrong object. THE RULING: a curated question is never removed for being old, for looking settled, or for being thinly traded — freshness is a TREATMENT, never a filter. WHY, and it is not a lowered bar: a thin market on a real question IS the product, because the places a reader could otherwise go do not have that question at all; what makes an old number dangerous is presenting it as a current one, and the honesty treatment already solves that completely, so deleting the card solves the danger by deleting the value. THE SETTLED CASE IS WORSE THAN THE OLD ONE: `propIsResolved` INFERS settlement from a probability at a rail, which on an illiquid market is a guess, and inferring settlement then HIDING the card is a wrong guess the reader cannot notice — so it is labelled "Looks decided", the strength of the word matching the strength of the evidence, and real settlement detection is owed by the ingest lane (flagged to lane1). WHAT THE TIMESTAMP MEANS, traced to the query and none of the three things Alex named: `age_hours` is `MAX(futures_odds_snapshots.captured_at) WHERE probability IS NOT NULL`, and every refresh writes a snapshot whether or not the number moved, so it is THE LAST TIME A PROBABILITY FOR THIS QUESTION REACHED US — not created, not `futures_outcomes.last_updated` (measured a month stale against running snapshots on day 1, which is why the route refuses to read it), not last traded (we receive no trades). AND ITS LIMIT: "32 hours" has two causes the number cannot separate — quoted-and-untraded, or not covered by our reader — so the copy says "we have not seen a new number", a fact about our knowledge, rather than "nobody has traded this", which would invent the half we do not have. THREE OBLIGATIONS ON THE TREATMENT: per card and never per section (the card is as fresh as its OLDEST printed outcome and names WHICH are old when only some are); the UNIT defined once under the section because a definition repeated per card is a footnote nobody reads; and a live card says NOTHING, because a healthy card that keeps apologising teaches the reader the apology is decorative. THE RIFF IS OPEN: `FreshnessVariant` is a real seam on the shipped component (`labelled` default / `sentence` / `dot`) and the artifact renders all three FROM THAT COMPONENT with the same real data, because a drawing of an alternative proves nothing about the page. GENERAL FORM: a surface may change how loudly it says something is old; it may not decide on the reader's behalf that they would rather see nothing (Alex ruling relayed through the UX-P154 runner directive, executed by the program-ux lane)
- [147](rulings/147-an-event-shows-through-the-event-card-and-the-whole-card-is-the-target.md) — 2026-08-28 — A SURFACE THAT SHOWS AN EVENT SHOWS IT THROUGH **THE** EVENT CARD, AND THE WHOLE CARD IS THE TARGET. Alex's words on the UX-P152 artifact's panel 4: "it kinda feels like we're reinventing the event card inside the tournament product", with the instruction "no 'See more on this match' link row — the whole match card is clickable, exactly like every other card in the product; the tournament list uses THE standard event-card component, full stop." EXTENDS ruling 047, whose acceptance was already "the league page renders the SHARED event card"; this says the same of every other surface and adds the interaction half. THE CASE: the tournament match list drew its own bordered container, its own dividers and its own tap behaviour — the row was a `<button>` opening an accordion, and inside the accordion was a link reading "See more on this match". Everywhere else a card IS a link, so a reader who taps a match card gets an accordion, learns this list works differently from every other list on the site, and pays that on every row for a fact worth one. THE RULING: one `<Link>` wrapping everything, no link row and no accordion between the tap and the page; where there is nothing to navigate to the card renders INERT (no anchor, no pointer, no hover lift), because a card that looks pressable and is not is worse than one that plainly is not; the card ANNOUNCES WHICH COMPONENT DREW IT via `data-testid="event-card"` emitted by `EventCardShell` and nothing else, since "this surface renders the shared card" is a DOM claim unanswerable from the DOM unless the shared card marks itself (a guard asserts exactly one emitter in the tree); and live/finished/hover treatment is decided in ONE place so two surfaces cannot drift into two ideas of what a live card looks like. WHAT "THE COMPONENT" MEANS HERE, and the cost of the literal reading recorded so Alex can overrule it knowing the price: rendering `EventCard` itself would resolve faces from the team name at render time (`espnTeamLogoByName`, `flagUrl`, `teamColorStyle`) against Alex's ruling 8 that a player's face is PINNED IN THE REGISTER and never resolved client-side — a team-logo lookup on two tennis players is how a basketball crest ends up beside Sabalenka; it has no seat for a SEED or the title chip (rulings 1 and 8 on this list) and would print "Alcaraz at Bellucci" for a match at nobody's home; and it keys off `event.id`, which 28 of the register's fixtures do not have, so those rows would have NO CARD AT ALL. So what shipped is `EventCardShell` — link, card, marker and state treatment shared, contents left to the surface — and if the literal version is wanted anyway the change is `EventCard` growing a `sides` slot, not a second card growing features. A MOVED FEATURE LOSES ONE HALF UNLESS BOTH ARE GUARDED: where-to-watch went to the event page (ruling 7's "detail view" is now the page the tap arrives at) with the POSITIVE held in `tournamentExtensions.test.tsx` and the NEGATIVE — that it did not quietly return to the row, including in the per-match case — in `tournamentMatches.test.tsx`; ruling 6's one sentence moved onto the card, where it fires only when it adds something the numbers cannot say. GENERAL FORM: interaction is part of the component, not part of the page — a surface that reimplements how a card is pressed has reimplemented the card, whatever it reused of the styling, and the reader pays in the one currency they cannot get back, which is knowing what will happen when they touch something (Alex ruling relayed through the UX-P154 runner directive, executed by the program-ux lane)
