...
46. **ORM attribute assignment lost when mixed with Core SQL updates** — Setting `event.espn_id = value` via ORM, then doing `session.execute(update(Event).where(...).values(win_probability_sources=...))` via Core SQL in the same session can cause the ORM change to silently not persist. Use Core SQL for both, or ensure explicit flush between ORM and Core writes. Same class of bug as gotcha #8 but for non-JSONB columns.
