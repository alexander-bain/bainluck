...
- ✅ Discover email ground-truth diagnostics: Polymarket email-highlight audit loader accepts stable `Audit Export` headers and URL/path env vars, supports private Google Sheets via service-account auth, records raw/loaded row counts, latest email date, and stale status, and surfaces private-export HTTP errors in `/admin/discover-quality` instead of breaking debug. Admin now splits Polymarket email misses into an editorial audit panel with bucket counts, sheet scores, hooks, DB trace entry points, and recommended actions. Email hits remain audit-only and do not directly boost ranking.
