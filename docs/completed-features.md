...
- ✅ Discover email ground-truth diagnostics: Polymarket email-highlight audit loader accepts stable `Audit Export` headers and URL/path env vars, records raw/loaded row counts, latest email date, and stale status, and surfaces private-export HTTP errors in `/admin/discover-quality` instead of breaking debug. Email hits remain audit-only and do not directly boost ranking.
