...
- ✅ **Today’s Challenge focused web flow** — Web Discover now opens Today’s Challenge as a focused modal, presents one Higher/Lower question at a time, auto-advances after answers, shows set progress/completion, and records first-party `challenge_start` / `challenge_complete` engagement for admin rollups.
