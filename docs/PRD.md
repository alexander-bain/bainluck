...
- **The latency charter (added 2026-08-24, Alex ruling; re-expressed 2026-08-26, Alex ruling 137):** the latency program is graded on **the speed a user actually feels** — the cold, user-volume paths, which open every one of its reports with deltas per cycle: **Discover first-load p50, Sports / Browse / My Stuff first-load p50, and cold search p50**. Alex's words: *"stop bragging about warm searches — a tiny fraction of searches will be warm."* Warm-hit numbers (feed p50 warm, typeahead p50 warm) are still measured and keep their series, but they are supporting evidence and **never lead a report again**. Bars: tab first-load p50 ≤ 1,000ms, cold search p50 ≤ 1,000ms, cold typeahead p50 ≤ 500ms, and a per-sample hard ceiling of 6,000ms (the native client's non-retryable `retryBudget`). Instrument work is permitted only when it unblocks a named decision, and any program's change that raises a measured beat cost past the declared threshold must carry a beat-cost budget line the Integrator enforces.
...
- Latency: the four tab first-load p50s (Discover / Sports / Browse / My Stuff) + cold search p50 open every latency report with deltas (ruling 137); warm numbers never lead. Standing target = whichever cold path the current baseline table shows as largest
