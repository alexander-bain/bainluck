# shellcheck shell=sh
# tools/lane-zdotdir/.zshrc — chain only. See `.zshenv` in this directory for why.
#
# Pointing ZDOTDIR at this directory MOVES zsh's whole startup search, it does not add to
# it: without this file, `$HOME/.zshrc` silently stops being read in every lane shell. That
...
if [ -n "${HOME:-}" ] && [ "${ZDOTDIR:-}" != "$HOME" ] && [ -f "$HOME/.zshrc" ]; then
    . "$HOME/.zshrc"
fi
