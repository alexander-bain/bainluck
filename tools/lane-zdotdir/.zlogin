# shellcheck shell=sh
# tools/lane-zdotdir/.zlogin — chain only. See `.zshenv` in this directory for why.
#
# Pointing ZDOTDIR at this directory MOVES zsh's whole startup search, it does not add to
# it: without this file, `$HOME/.zlogin` silently stops being read in every lane shell. That
...
if [ -n "${HOME:-}" ] && [ "${ZDOTDIR:-}" != "$HOME" ] && [ -f "$HOME/.zlogin" ]; then
    . "$HOME/.zlogin"
fi
