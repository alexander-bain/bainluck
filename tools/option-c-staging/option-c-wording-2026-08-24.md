...
UX-P125 (was UX-P122 item C). **APPLIED.** This is no longer a staging doc:
the functions below are imported from the shipped
`frontend/lib/calibrationPopulation.ts`, so what is printed here is what the
page renders — a divergence sweep, not a proposal.

- payload `generated_at`: `2026-08-24T05:36:13.606033+00:00`
...
**Amendment 6 is exactly this pair of numbers.** The pre-UX-P125 page passed
`7` (every normalized key) over `15` (rendered rows) — a fraction whose
halves came from two different populations. The numerator is now **6**, taken from the rendered rows, and this sweep asserts the section sentence agrees.
...
capped at 4), the counts-only tooltip, and the FULL member list the
expander carries. THE EXPANSION IS NEVER CAPPED — that is what makes the
collapsed form's "and N more" legal rather than a #2108 reprise.

### Baseball (`baseball`)

Renders **1.4pp** over n=153,876 (axis D, unchanged). Folds **4** payload categories: **4** published, **0** not.
...
tooltip (counts only — never the member wall):
...
Renders **1.7pp** over n=17,938 (axis D, unchanged). Folds **8** payload categories: **4** published, **4** not.

```
This row pools 4 published categories (tennis, tennis_atp_canadian_open, tennis_atp_cincinnati_open and tennis_wta_canadian_open) and 4 unpublished, and is measured over traded outcomes only. The API publishes 1.97pp for “tennis” over 47,084 outcomes — that figure covers the “tennis” category alone, over the whole population.
```

tooltip (counts only — never the member wall):

```
This row pools 4 published categories and 4 unpublished, and is measured over traded outcomes only. The API publishes 1.97pp for “tennis” over 47,084 outcomes — that figure covers the “tennis” category alone, over the whole population.
```

<details><summary>full member list (8)</summary>

- published in `by_category` (4): tennis, tennis_atp_canadian_open, tennis_atp_cincinnati_open, tennis_wta_canadian_open
- not published (4): tennis_atp_washington_open, tennis_wta_cincinnati_open, tennis_wta_monterrey_open, tennis_wta_washington_open
...
Renders **3.1pp** over n=5,810 (axis D, unchanged). Folds **5** payload categories: **1** published, **4** not.
