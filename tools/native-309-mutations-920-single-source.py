...
MUTANTS = [
    ("M1 tie/older readings admitted", "Components/OddsChartView.swift",
     ...
     "let edge = edges[venue], frame.date >= edge.addingTimeInterval(-86400) else { continue }"),
    ("M2 unserved source minted", "Components/OddsChartView.swift",
     "let edge = edges[venue], frame.date > edge else { continue }",
     "case let edge = edges[venue] ?? .distantPast, frame.date > edge else { continue }"),
    ("M3 blend p stands in for the venue value", "Components/OddsChartView.swift",
     ...
     "_ = value; extended.append(ChartDataPoint(date: frame.date, probability: frame.homeProbability, source: venue))"),
