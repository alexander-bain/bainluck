...
Failure display: `retry_wait` identifies the issue, preserved run, runtime error and UTC retry deadline. The 15-minute delay applies only to that issue; other eligible diagnoses may continue. Three failed attempts become `needs_attention`. A report written before a runtime shutdown failure is still incomplete, not delivered. After independent disposition, the coordinator may mark a stopped worker's `state.json` entry `status: reviewed` with its disposition and run path to prevent repeating reviewed work; this does not accept its patch. Preserve the failure fields and original artifacts. `REVIEWED` alone acknowledges artifact review and does not change retry eligibility.
