...
# ---------------------------------------------------------------------------
# CAL-P097 / CERT-403B — the exact-0.5000 PAIR spike, and the one reason this
# predicate is defined here instead of in the fold that measures it.
#
# CERT-403B BLOCKED the staged half-spike exclusion on a MEASUREMENT defect, not
# an arithmetic one. The proposal's rule is pair-scoped — exclude a leg only when
# BOTH legs of an exactly-two-leg named Over/Under market are 0.5000 — and its own
# criterion 2 requires a LONE 0.5000 leg to survive, because a lone even-money
# price on a real market is an ordinary price. But the 12.14 pp artifact that was
# offered as its predicted result was generated by ``fold_arbitrate_bbq.py`` with
# ``AND ROUND(fo.opening_probability, 4) <> 0.5000`` — a bare per-leg filter with
# no partner test, no pair-size test and no name test. Reconciled by the cert:
# baseline 6,778 legs, committed "after" 4,686, so 2,092 legs removed, against
# only 1,826 exact-0.5000 legs in the coherent named-pair class. **266 extra legs
# — precisely the lone-leg class criterion 2 says must not be swallowed.** So
# 12.14 is not this rule's ECE; it is a different, broader rule's ECE.
#
# The structural fix is not "be more careful next time". It is that the fold and
# the shipping builder must be UNABLE to disagree: the shape clauses below are
# the single definition, both sites render them from these same strings, and
# ``tests/test_half_spike_pair_exclusion.py`` asserts the two renderings are
# identical modulo alias. ``fold_spike_provenance.py`` already established this
# pattern by asserting at import that its tolerance still equals the shipped one;
# this goes one step further and shares the predicate text itself.

#: The per-market shape counts the pair predicate is defined over. Keyed by the
#: output column name; each value formats with ``o`` = the outcome alias. These
...
#: The exact value. Scoped to 0.5000 and deliberately NOT a tolerance band: the
...
#: That widening is a separate ruling with its own census (staged spec §5).
...
def half_spike_pair_market_predicate(shape_alias: str = "mrs") -> str:
    """TRUE for a market whose BOTH legs are the 0.5000 placeholder pair. Pure.

    The third and fourth clauses together are what make the predicate
    self-evidencing rather than a guess. ``hs_half_legs = 2`` with
    ``hs_n_outcomes = 2`` says *every* leg of this market is exactly 0.5000,
    which is the arithmetic signature of the writer defect: the Over leg took
    0.5 from an untraded market's precomputed price and the Under leg was
    written as its complement ``1 - 0.5``. A market with ONE 0.5000 leg fails
    ``hs_half_legs = 2`` and is kept — that is criterion 2, in the predicate
...
#: The CELL this exclusion ships scoped to, and why it is not global.
#:
#: CERT-403B's second P1: the spec itself required a population-wide census
#: before an unscoped rule, and CAL-P095 supplied the first out-of-cell control —
#: ``soccer/quantity`` moves 8.51 / n=5,749 -> 8.92 / n=3,761, a **+0.41 pp
#: REGRESSION**, while removing 1,988 legs. The same exact-value mass exists at
#: nearly the same share in two cells and has OPPOSITE calibration effect, so a
#: global predicate would knowingly degrade a measured cell. Widening to a second
#: cell needs that cell's own census and its own ruling.
HALF_SPIKE_PAIR_CELL_SOURCE = "polymarket"
HALF_SPIKE_PAIR_CELL_CATEGORY = "baseball"
HALF_SPIKE_PAIR_CELL_MARKET_TYPE = "quantity"

HALF_SPIKE_PAIR_RULE_TEXT = (
    "Excludes both legs of a resolved Polymarket baseball/quantity market that has "
    "exactly two outcomes named Over and Under and whose BOTH opening probabilities "
    "are exactly 0.5000 — the Gamma placeholder pair, where the Over leg took 0.5 "
    "from an untraded market and the Under leg was written as its arithmetic "
    "complement with no quote of its own (CAL-P094: 924/924 such Under legs carry "
    "neither a bid nor an ask). A LONE 0.5000 leg is an ordinary even-money price "
...
#: The shipped half-spike leg test, as one expression. Named so the fold can ask
#: for the population WITHOUT it (``half_spike_pair_enabled=False``) instead of
#: rebuilding a population of its own — CERT-406B's fix-sketch, verbatim: "run
#: both baseline and proposed readings through the same frozen shared-builder
#: population through ``deduped``".
...
PUBLISHED_ROW_JOIN = """LEFT JOIN mode_prices mp
...
def published_row_predicate(*, half_spike_arm: str) -> str:
    ...
    ----------------------------------------------
    CERT-403B blocked an exclusion whose evidence executed a DIFFERENT predicate
    from the one it proposed. CERT-406B then blocked its rework because the
    measurement executed a different POPULATION from the one that publishes. Both
    findings are the same shape: a second copy of a rule, written by hand, that
    agreed with the original on the day it was written. The only closure is to
    stop having a second copy — the fold imports this, the payload counter
    renders this, and the published rows come out of this. A drift between them
    is now a syntax error rather than a number nobody re-checks.

    ``ro`` and ``mp`` are the required aliases: ``normalized ro`` plus
    ``PUBLISHED_ROW_JOIN``.
    """
    return f"""ro.is_liquid AND NOT ro.is_poly_placeholder
    ...
    return f"""{leading_ctes}market_info AS (
            ...
            market_result_shape AS (
                SELECT fo.market_id,
                    ...
                    -- CAL-P097 / CERT-403B: the exact-0.5000 pair shape. Four
                    -- more FILTER aggregates over a GROUP BY that already scans
                    -- every outcome — no new scan, no new join. Rendered from
            ...
            -- CAL-P097 / CERT-403B: markets whose BOTH legs are the 0.5000
            -- placeholder pair, inside the ONE cell this ships scoped to. The
            -- shape test and the cell scope are separate clauses on purpose:
            -- the shape is shared verbatim with the fold, the scope is a
            -- per-site decision that CERT-403B required be made explicitly.
            half_spike_pair_markets AS (
                SELECT mrs.market_id
                FROM market_result_shape mrs
                JOIN market_info mi ON mi.market_id = mrs.market_id
                WHERE {half_spike_pair_market_predicate('mrs')}
...
{_virtual_market_ctes(frozen_vm_roster)}
            ...
            ranked_outcomes AS MATERIALIZED (
                SELECT
                    ...
                    -- CAL-P097 / CERT-403B: this leg is one of the two 0.5000
                    -- legs of a named Over/Under placeholder pair. The market
                    -- membership already implies the leg value (a market only
                    -- qualifies when EVERY one of its two legs is 0.5000), but
                    -- the leg test is restated rather than implied — it is the
                    -- clause criterion 2 is about, and a reader auditing why a
                    -- particular row left the curve should not have to derive
                    -- it from an aggregate two CTEs away.
                    --
                    -- CAL-P099 / CERT-406B: this ONE expression is the whole
                    -- on/off switch for the rule, and that is deliberate.
                    -- ``half_spike_pair_enabled=False`` renders it ``false``, so
                    -- every downstream gate — ``field_completeness``'s two
                    -- survivor filters, ``deduped``, and the published-removal
                    -- counter — becomes a no-op WITHOUT a single other
                    -- character of the chain changing. That is how the
                    -- before/after reading is taken through the same builder
                    -- rather than through a fold that re-implements it, which
                    -- is the BLOCK: "the artifact proves arithmetic over 6,778
                    -- raw truth-eligible legs, not the before/after identity of
                    -- the published bucket rows."
                    --
                    -- MEASUREMENT-ONLY. The default is the shipped rule; the
                    -- fold is the only caller that passes False, and the
                    -- fingerprint moves either way because this text is inside
                    -- the hashed function.
            ...
            --
            -- The BLOCK: "the payload's new ``half_spike_pair_filter.excluded``
            -- is counted candidate-side from ``normalized``, while published
            -- bucket totals come from ``deduped``; therefore criterion 4
            -- ('published totals move by the excluded count and no more') is
            -- neither measured nor structurally guaranteed."
            --
            -- It is not guaranteed because the two counts answer different
            -- questions. A candidate flagged ``is_half_spike_pair`` may ALSO be
            -- illiquid, a placeholder, a malformed binary, or a losing ``rn``
            -- side — in which case it was never going to publish and the
            -- exclusion removed nothing. Candidate-side 4,982 and published-side
            -- 4,982 are not the same claim, and only the second one is what a
            -- reader of the curve sees move.
            --
            -- So this counts the rows the exclusion ACTUALLY removed from the
            -- published population: flagged, and passing every OTHER predicate
            -- ``deduped`` applies. It shares that predicate TEXTUALLY —
            -- ``published_row_predicate`` renders both, differing in the one
            -- half-spike arm — because a restated copy is precisely the defect
            -- class CERT-403B and CERT-406B have now each found once.
            half_spike_pair_removed AS (
...
_COVERAGE_RUNG_PREDICATES: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        # CAL-P097 / CERT-403B. Folded into the EXISTING rung rather than given a
        # new one, and that is a deliberate grain decision, not an omission. These
        # rungs are a coarse 7-bucket partition over ~15 per-outcome flags — the
        # weather wide-spread flag lives inside ``structural_artifact``, the poly
        # placeholder inside this one — and the pinned corpus
        # (``calibration_coverage_bridge_contract.json``) enumerates the rung NAMES
        # with reconciling totals, so a new name is a corpus-wide change that says
        # nothing new about the population. The 0.5000 pair IS phantom liquidity:
        # the same Gamma-placeholder mechanism as ``is_poly_placeholder``, caught
        # by its arithmetic signature instead of by its missing book.
        #
        # Gate 6 of the staged spec ("the excluded population is REPORTED, not
        # silently dropped") is satisfied at the grain it asks for by the
        # per-rule count + reason string in the exclusions payload below, which is
...
def _main_futures_sql(*, frozen: bool = False) -> str:
    ...
    return (
            ...
            + """ AS n,
                ...
                -- answer criterion 4, which is about the rows a reader of the
                -- curve loses. Both are reported; neither is derived from the
                -- other.
...
async def compute_calibration_payload(db, *, runner=None) -> dict:
    ...
    with contextlib.nullcontext():
        ...
        # CAL-P097 / CERT-403B gate 6: the exact-0.5000 pair exclusion count.
        #
        # ``getattr`` with a default, NOT ``rows[0].half_spike_pair_excluded``
        # like its neighbours above. This column is NEW, and a census row that
        # predates it has no such attribute — direct access raises
        # ``AttributeError`` and takes the WHOLE payload build down, which for a
        # reporting-only counter is a spectacularly bad trade. The neighbours are
        # safe only because every row shape in flight has caught up with them.
        # Same reasoning the ``representative_tie_broken`` comment below records
        # for a genuinely-absent column, and the same lesson as reading a new ORM
        # column off a row that was loaded before it existed.
        _hsp = getattr(rows[0], "half_spike_pair_excluded", None) if rows else None
        half_spike_pair_excluded = int(_hsp) if _hsp is not None else 0
        # CAL-P099 / CERT-406B: the published-side companion.
        #
        # ``_int_or_none``-shaped rather than ``or 0``, and deliberately: a
        # census row written before this column existed genuinely does not know
        # how many published rows the exclusion removed, and reporting 0 there
        # would assert "it removed nothing" — which is the strongest possible
        # claim about criterion 4 and the one nobody measured. Unknown says
        # unknown (gotcha #53).
        _hspr = (
            getattr(rows[0], "half_spike_pair_published_removed", None)
            if rows
            else None
        )
    ...
    response = {
        ...
        # CAL-P097 / CERT-403B gate 6. The staged spec's sixth criterion is that
        # the excluded population is REPORTED rather than silently dropped, so a
        # reader can see what was removed and on what grounds — the standing rule
        # against silent caps. ``applies_to`` names the CELL, not just the source,
        # because the scope IS the finding: the same mass in soccer/quantity moves
        # that cell +0.41 pp and is deliberately left in.
        "half_spike_pair_filter": {  # CAL-P097 (CERT-403B)
            "applies_to": (
                f"{HALF_SPIKE_PAIR_CELL_SOURCE} "
                f"({HALF_SPIKE_PAIR_CELL_CATEGORY}/"
                f"{HALF_SPIKE_PAIR_CELL_MARKET_TYPE} only)"
            ),
            "rule": HALF_SPIKE_PAIR_RULE_TEXT,
            # CANDIDATE-side: rows in ``normalized`` carrying the flag. This is
            # what the rule matched.
            "excluded": half_spike_pair_excluded,
            # PUBLISHED-side: how many of those would have reached the curve
            # had the exclusion not fired — i.e. what a reader actually loses.
            # CERT-406B: criterion 4 is "published totals move by the excluded
            # count and no more", and the candidate count cannot answer it,
            # because a flagged row that was ALSO illiquid, a placeholder, or a
    ...
    * :data:`REPRESENTATIVE_TIE_AUTHORITY` — hashed explicitly rather than left
    ...
    try:
        ...
        source = (
            ...
            # CAL-P099: the third time this docstring's own rule has been
            # learned the hard way, and the first time by the edit that was
            # reading the docstring.
            #
            # CERT-406B's fix hoists ``deduped``'s WHERE clause into
            # ``published_row_predicate`` so the published-row counter renders
            # the SAME text instead of a hand-written copy. That is the right
            # shape — and, done naively, it QUIETLY REMOVES the entire published
            # -population predicate from this digest, because the text moved out
            # of ``_calibration_population_ctes``'s source and into a callee's.
            # Every exclusion in the curve would then be editable without moving
            # the fingerprint. Measured before this line existed: mutating the
            # predicate left the digest at 23ea3b8cf453c4f052cc6cf05e02c181,
            # unchanged.
            #
            # The general rule, restated because a refactor is exactly when it
            # gets broken: hashing a function's source covers that function,
            # never what it calls. Hoisting SQL into a helper is therefore a
            # fingerprint change, always, and the helper joins this list in the
    ...
    return input_fingerprint(
        ...
        # CAL-P097 (#2212): the 0.5000-pair exclusion's SQL-shaping inputs,
        # hashed BY VALUE, on the instruction this docstring already gives —
        # "new SQL-shaping inputs belong on this list explicitly rather than
        # being assumed covered".
        #
        # They are NOT covered by the ``source`` term above, and the reason is
        # the same one COVERAGE_CENSUS_ENABLED cost a queue to learn. Hashing a
        # function's source covers that function, never what it calls and never
        # the VALUES it interpolates: ``inspect.getsource`` returns the literal
        # text ``{HALF_SPIKE_PAIR_CELL_CATEGORY}``, not ``baseball``. So without
        # this line, editing the cell scope from baseball to soccer would change
        # which markets leave the published curve while leaving the digest
        # identical — a carried read banked under one population resumable by
        # code publishing a different one, which is precisely what this digest
        # exists to make impossible.
        #
        # ``half_spike_pair_shape_columns`` is hashed by its RENDERED text rather
        # than by the dict, because the dict's repr is not what reaches the SQL
        # and the renderer is a callee whose source the term above does not
        # cover either.
        f"half_spike_value={HALF_SPIKE_EXACT_VALUE}",
        f"half_spike_cell={HALF_SPIKE_PAIR_CELL_SOURCE}/"
        f"{HALF_SPIKE_PAIR_CELL_CATEGORY}/{HALF_SPIKE_PAIR_CELL_MARKET_TYPE}",
        half_spike_pair_shape_columns("fo"),
        half_spike_pair_market_predicate("mrs"),
        # CAL-P099: two module-level SQL constants, so neither is in ANY hashed
        # function's source — ``inspect.getsource`` returns the literal text
        # ``{PUBLISHED_ROW_JOIN}``, never its value. Both shape the emitted
        # statement, which is the only class of input that can silently change
        # which rows publish, so both are hashed BY VALUE rather than booked as
        # new holes in the coverage census.
        #
        # ``PUBLISHED_ROW_JOIN`` carries the mode-price LEFT JOIN that
        # ``deduped``'s multi-row arm tests (``mp.vm_id IS NULL``); drop or
        # widen it and the published population moves.
        # ``_HALF_SPIKE_FLAG_SQL`` IS the half-spike rule — the single
