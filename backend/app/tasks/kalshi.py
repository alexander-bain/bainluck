...
from app.utils.kalshi_empty_book import book_refutes_price  # noqa: E402  # #5121, moved #6532
...
# #6532 moved the book-refutation rule out to `app.utils.kalshi_empty_book`,
# unchanged, because the serve layer must ask the identical question of a stored
# price and a price policy that exists twice drifts. The write-side call below is
# the one #5121 shipped; the read-side callers are in
# `app.utils.futures_unsupported_price`.
...
def _kalshi_yes_probability(
    ...
    captured as closing lines and inflated calibration MCE (#181). Priority:
      ...
      2. else a real trade the LIVE BOOK DOES NOT REFUTE -> last_price
         ...
         — see :func:`app.utils.kalshi_empty_book.book_refutes_price`, #5121)
    ...
    if (
        ...
        and last_price > 0
        and not book_refutes_price(yes_bid, yes_ask, last_price)
    ):
        return last_price
    if (
        yes_bid is not None
