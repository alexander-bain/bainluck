...
#: ⚠️ Only `1000` occurs in the corpus today: a census of all 1,677 open tennis
#: markets on 2026-08-29 found it exactly TWICE, both of them the duplicate pair
#: above, and found no `125`/`250`/`500` at all. Those three are therefore a
...
async def list_tennis_tournament_concepts(
    ...
    *,
    ...
    for ms in groups.values():
        ...
        # This is not guessing a date we do not have (the rule the `start_date`
        # comment below states) — it is reading one we do have, off another
        # rendering of the same tournament.
