...
1. **A survivor must be UNSETTLED.** When every row in the group is settled the
   rule stays silent — it cannot tell which result belongs to which question,
   and guessing would attach a real grade to the wrong one. This is what keeps
   it off ``189`` (Heisman, ``Josh Hoover`` under two Kalshi tickers, both
   graded) and ``109506`` (``Man I Need``, both graded).
...
def superseded_name_twins(
    ...
    *,
    ...
    for group in by_name.values():
        ...
        has_live_anchor = any(
            not _is_definite_loss(resolution_source_of(r), is_winner_of(r))
