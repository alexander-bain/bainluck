...
# --------------------------------------------------------------------------
# Discover: same-domain concept cards do not sit shoulder to shoulder (#2602)
# --------------------------------------------------------------------------
#
# EDITORIAL POLICY, NOT A TRUTH OR IDENTITY CORRECTION. Every card this pass
# touches is a real, eligible card and every one of them is still served: the
# pass reorders, it never drops, duplicates, rescores or re-keys. It exists
# because concept cards of one domain score off the same small formula
# (`_score_event_concept`), so they land on the same two or three score values
# and the score sort seats them as one block — four fight cards in a row at the
# tail of the saved 2026-09-17 page.

#: Slots at the head of the list this pass never reads or writes. It is the
#: composition window: the lead, the marquee seats, the first-page quota re-pick
#: and the first-page quality floor all write inside it, and none of them should
#: have to reason about one more writer. Passed in by the caller so the two
#: numbers cannot drift.
DISCOVER_SPACING_PROTECTED_PREFIX = 20

#: The most slots any single card may end up from where the score sort put it,
#: in EITHER direction. Four is the smallest bound that dissolves both saved
#: tails (2026-09-17 07:38Z run of five, 16:18Z run of four) as far as their own
#: candidate pool allows; one and two leave a run of three, three leaves a pair
#: mid-tail. It is a hard bound, asserted on the output, not a target.
DISCOVER_SPACING_MAX_DISPLACEMENT = 4

#: Concept domains a reader sees as the same kind of card. The key is the
#: adapter's stable ``data.domain`` — never ``sport_label`` or a title, which are
#: display strings and may be corrected. Absent from this map, a domain is its
#: own family.
_DISCOVER_SPACING_DOMAIN_FAMILY = {"ufc": "combat", "boxing": "combat"}

#: futures categories that read as the same subject as a concept family, so they
#: are not used as the separator between two of its cards. They are never moved
#: on their own account.
_DISCOVER_SPACING_FAMILY_CATEGORIES = {"combat": frozenset({"mma", "boxing"})}


def discover_spacing_family(item: dict) -> str | None:
    """The spacing family of a card, or ``None`` when the pass ignores it.

    Only ``concept`` cards have one. Everything else — futures, bundles, events,
    tournaments — is never deferred by this pass.
    """
    if item.get("type") != "concept":
        return None
    domain = ((item.get("data") or {}).get("domain") or "").strip().lower()
    if not domain:
        return None
    return "concept:" + _DISCOVER_SPACING_DOMAIN_FAMILY.get(domain, domain)


def _spacing_is_anchor(item: dict) -> bool:
    """Cards whose slot is somebody else's guarantee: never moved, never crossed."""
    data = item.get("data") or {}
    return bool(
        data.get("status") == "live"
        or data.get("is_marquee")
        or data.get("is_major")
        or data.get("discover_marquee_final")
        or data.get("marquee_whathit")
    )


def _spacing_separates(item: dict, family: str) -> bool:
    if discover_spacing_family(item) == family:
        return False
    data = item.get("data") or {}
    category = (data.get("llm_sport_category") or data.get("sport") or "").lower()
    related = _DISCOVER_SPACING_FAMILY_CATEGORIES.get(family.split(":", 1)[-1], ())
...
def space_discover_concept_families(
    items: list[dict],
    *,
    protected_prefix: int = DISCOVER_SPACING_PROTECTED_PREFIX,
    max_displacement: int = DISCOVER_SPACING_MAX_DISPLACEMENT,
) -> tuple[list[dict], dict]:
    """Defer a concept card that would directly follow one of its own family.

    One left-to-right pass over the canonical, already-composed order, BEFORE
    pagination. When the next card shares a spacing family with the card just
    placed, the nearest later card that separates them is served first and the
    concept card waits one slot. Rules, all asserted by the guard tests:

    * the multiset of cards is unchanged — nothing dropped, added or rescored;
    * ``items[:protected_prefix]`` is returned byte-for-byte;
    * no card ends more than ``max_displacement`` slots from its input index;
    * an anchor (live, marquee, major, kept final) keeps its exact index — it is
      never deferred and the search for a separator never crosses one;
    * when the pool has no separator left — an all-one-family list, or the last
      cards of the tail — the run is served as it stands and counted in
      ``unresolved_adjacent``. A run the pool cannot break is a fact about the
      pool (gotcha #53: it must not read the same as a clean pass);
    * it is a pure function of the input order: no clock, no request, no user,
      no ``limit``/``offset``, so every page size and every offset slices one
      and the same list.
    """
    meta = {
        "moved": 0,
        "max_displacement_seen": 0,
        "unresolved_adjacent": 0,
        "bound": max_displacement,
    }
    n = len(items)
    if n <= protected_prefix + 1 or max_displacement <= 0:
    ...
    while pending:
        ...
        family = discover_spacing_family(head)
        previous_family = discover_spacing_family(out[-1]) if out else None
        take = 0
        if (
            family is not None
            ...
            for offset_in_pending in range(1, len(pending)):
                cand_index, cand = pending[offset_in_pending]
                if cand_index - slot > max_displacement:
                    break
                if _spacing_is_anchor(cand):
                    break  # never pull a card across an anchor
                if _spacing_separates(cand, family):
                    take = offset_in_pending
                    break
        if (
            take == 0
            ...
            meta["unresolved_adjacent"] += 1
        index, item = pending.pop(take)
        if index != slot:
            meta["moved"] += 1
            meta["max_displacement_seen"] = max(
