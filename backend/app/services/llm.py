...
def generate_golf_live_commentary(prompt: str) -> Optional[str]:
    ...
    system_prompt = (
        ...
        "shows probabilities, not betting odds. It reports WHAT JUST MOVED since "
        "the last update. STRICT RULES:\n"
        "- Use ONLY the numbers, names, holes, and scoring events in the user's "
        "data. Do NOT invent or infer any score, position, hole number, name, or "
        "event not explicitly given. If the data does not state a hole number, do "
        "NOT mention a specific hole.\n"
        "- The percentages are model WIN/finish/market PROBABILITIES. Call them "
        "'probability'/'chances' — never 'odds', 'lines', or betting terms.\n"
        "- When a scoring event and a probability move are given together, you may "
        "connect them (e.g. 'as X birdied the 17th, the U.S. chances rose from 40% "
        "to 42%'), but ONLY using the exact numbers/holes provided. Frame it as "
        "co-occurring ('as'/'after'/'while'); do not overstate causation.\n"
        "- Report the market moves that are given (region/group/prop probabilities "
        "shifting), not just one golfer's win probability. Prefer the movements "
        "with scoring context. Give the from->to numbers.\n"
        "- No betting advice, no predictions beyond what the numbers state.\n"
        "- 2-4 tight sentences, plain and factual. No preamble, no hype, no "
...
def enrich_event_taxonomy(
    ...
    prompt = (
        ...
        "relevant tags from EACH of the 4 dimensions. Use ONLY the allowed values.\n\n"
        "GUIDANCE:\n"
