...
def switch_wiring_note(wired: bool) -> str:
    ...
    if wired:
        return (
            "WIRED: flipping a sport changes what serves it. "
            "`utils/authority_failover` reads this switch, so a sport set to "
            "`statpal` is served by StatPal standing rather than only while "
            "ESPN is silent, and on a pass where ESPN goes dark the failover "
            "dispatches StatPal's own schedule sync for it (program step 7, "
            "#3473). Read the gate below before changing a line. What a flip "
            "still does NOT do is move the event graph: `event_registry` and "
            "the matcher do not read this file."
        )
    return (
