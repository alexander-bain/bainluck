...
WIN_PROB_SOURCES = {
    "betting": {
        ...
        "color": "#374151",
        "dash_pattern": None,
        "description": "Consensus win probability derived from sportsbook moneyline odds, aggregated across multiple sportsbooks by The Odds API.",
        "methodology": "Moneyline odds from each sportsbook are converted to implied probabilities, then the vig (overround) is removed. The median probability across all reporting sportsbooks is used as the consensus.",
    ...
    "stat_model": {
        ...
        "sports": [
            "americanfootball_nfl", "americanfootball_ncaaf",
            "basketball_nba", "basketball_ncaab", "basketball_wncaab",
            "icehockey_nhl", "baseball_mlb", "baseball_mlb_preseason",
            "baseball_ncaa",
        ],
        "color": "#8b5cf6",
        "dash_pattern": "4 4",
        "description": "An open win probability model using sport-specific methodology. Clock sports (NFL, NBA, NHL) use a normal distribution model inspired by nflfastR. Baseball uses a Poisson run-scoring model based on Tom Tango's methodology.",
        "methodology": "Clock sports: Based on Hal Stern's research. Final margin modeled as N(score_diff + spread × remaining_fraction, base_std × √remaining_fraction). Baseball: Based on Tom Tango's 'The Book'. Remaining runs modeled as Poisson(runs_per_half_inning × half_innings_remaining). Normal approximation gives P(home wins) accounting for asymmetric half-innings, walk-off rule, and home field advantage (~54% historical home win rate).",
