...
# ---------------------------------------------------------------------------
# Fair-fight accuracy comparison — paired MCE on shared markets
# ---------------------------------------------------------------------------

_fair_fight_cache: dict = {"data": None, "timestamp": 0}
_FAIR_FIGHT_TTL = 3600  # 1 hour

# Minimum shared markets to report a pair — below this, noise dominates.
_MIN_SHARED = 100


def _compute_mce(probs: list[float], outcomes: list[bool]) -> float | None:
    """Compute Mean Calibration Error (MCE) from raw predictions and outcomes.

    Buckets predictions into 10 bins (0-10%, 10-20%, ..., 90-100%) and
    computes the mean absolute deviation between average predicted probability
    and actual win rate across all non-empty buckets.

    Returns MCE as percentage points (e.g. 3.2 means 3.2pp).
    """
    if not probs:
        return None
...
async def _query_futures_fair_fight(db: AsyncSession) -> list[dict]:
    """Paired MCE comparison for Kalshi vs Polymarket on futures markets.

    Finds resolved futures outcomes where the same question appears on both
    sources (matched via group_id or canonical_market_key), then computes MCE
    for each source on only the shared set.

    Uses calibration_probability (closing line) when available, falling back
    to opening_probability. Excludes low-quality resolution sources.
    """

    # Step 1: Find shared groups/keys between Kalshi and Polymarket.
    ...
    sql = text("""
        ...
        key_pairs AS (
            ...
            WHERE sq1.source = 'kalshi' AND sq2.source = 'polymarket'
              ...
              -- Exclude if already matched by group_id
              AND NOT EXISTS (
        ...
        -- Pull outcome-level data for matched markets
        matched_outcomes AS (
            SELECT
    ...
    # Compute paired MCE
    pair_key = "kalshi|polymarket"

    # Aggregate across all categories for the overall pair
...
async def _query_sports_fair_fight(db: AsyncSession) -> list[dict]:
    """Paired MCE comparison for prediction markets vs Odds API on sports events.

    For each source (kalshi, polymarket) that wrote win_prob_snapshots for
    completed events, finds events that also have closing betting lines from
    the Odds API, then computes MCE for both on only the shared events.
    """

    # Get closing lines from win_prob_snapshots for kalshi/polymarket,
    # paired with betting closing lines from the events table.
    sql = text(f"""
        WITH wp_closing AS (
            SELECT DISTINCT ON (wp.event_id, wp.source)
                wp.event_id, wp.source, wp.home_win_probability
...
async def fair_fight_comparison(
    ...
    For each pair of sources, finds markets they BOTH cover and computes
