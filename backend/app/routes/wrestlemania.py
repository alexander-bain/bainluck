...
async def _get_player(session, token: str | None):
    if not token:
        return None
...
async def _player_picks(session, player_id: int) -> list[dict]:
    result = await session.execute(
...
@router.get("/card")
async def get_card(x_player_token: str | None = Header(None)):
    async with get_session() as session:
        ...
        for match in matches:
            ...
            # All picks on this match (public)
...
@router.post("/picks")
async def submit_pick(req: PickRequest, x_player_token: str = Header()):
    if req.stake < 1:
    ...
    async with get_session() as session:
        ...
        # Check for existing pick on this match (delete-then-insert)
...
async def delete_pick(pick_id: int, x_player_token: str = Header()):
    async with get_session() as session:
        player = await _get_player(session, x_player_token)
        if not player:
            raise HTTPException(401, "Invalid player token")
...
async def get_odds_history(match_id: int):
    async with get_session() as session:
        outcomes_result = await session.execute(
...
async def resolve_match(req: ResolveRequest, secret: str = Query()):
...
async def add_match(req: AddMatchRequest, secret: str = Query()):
    _verify_admin(secret)
