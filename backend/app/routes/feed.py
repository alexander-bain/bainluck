...
async def get_feed(
    ...
    def _live_ttls(payload, *, oldest_artifact_age_s=None):
        ...
        a payload exists, which is exactly why the live bug could not be fixed
        there: the route cannot know a page holds an in-progress game until it
    ...
    def _live_bounded_last_good(key):
        ...
        Unbounded last-good is defensible for a page of futures and is not
        defensible for an in-progress score, so a LIVE payload past
        ``FEED_LAST_GOOD_MAX_AGE_LIVE_SECONDS`` is refused and the caller falls
    ...
    if not debug and not exclude_reviewed:
        ...
        try:
            ...
            if _fresh.is_failure or _stale.is_failure:
                ...
                _cache_status = "error"
                _lg = _live_bounded_last_good(_cache_key)
                if isinstance(_lg, dict):
                    out = dict(_lg)
                    _lg_fresh_ttl, _lg_stale_ttl = _live_ttls(_lg)
                    out["cache"] = build_feed_cache_metadata(
                        "last_good",
                        ttl_seconds=_lg_fresh_ttl,
                        stale_ttl_seconds=_lg_stale_ttl,
                        reason="redis_unavailable",
                        live=payload_contains_live_event(_lg),
                    )
                    _previous_at = _record_feed_timing(
    ...
    if _cache_key and _cache_status in ("miss", "error"):
        ...
        if not _is_build_leader:
            ...
            if isinstance(_coalesced, dict):
                ...
                out["cache"] = build_feed_cache_metadata(
                    "coalesced",
                    ttl_seconds=_co_fresh_ttl,
                    stale_ttl_seconds=_co_stale_ttl,
                    live=payload_contains_live_event(_coalesced),
                )
                _previous_at = _record_feed_timing(
    ...
    try:
        ...
        if (
            ...
            if _shared_cache_key != _cache_key:
                ...
                if _shared_payload is not None:
                    ...
                    # takes the LAT-P089 share and republishes under the
        ...
        # #2216: the built payload is the FIRST point in this route where the
        # page's own state is knowable, so this is where the live ceiling is
        ...
        if _cache_key and not _is_degraded_build and _shared_redis is not None:
            try:
                ...
                async def _publish_feed_cache(
                    ...
                    _json=_payload_json,
                    _key=_cache_key,
                    _fresh_ttl=_publish_fresh_ttl,
                    _stale_ttl=_publish_stale_ttl,
                ):
                    await _rc.bounded_redis_call(
                        lambda: _client.setex(_key, _fresh_ttl, _json)
                    )
                    await _rc.bounded_redis_call(
                        lambda: _client.setex(f"{_key}:stale", _stale_ttl, _json)
                    )
...
def _futures_feed_load_options() -> list:
    ...
    for it: see the #1698 note on `market_type`).
    ...
    LAT-P174 (#2143 residual) moved the columns THEMSELVES one level down, into
    `app/utils/futures_market_snapshot.py`, and this function now builds from
    them. That is the same rule applied once more rather than a second home for
    it: those tuples are simultaneously this query's load surface AND the wire
    format of the shared hydration artifact `_score_futures` reads, so a column
    added for a new read cannot land on the query and miss the snapshot either.
    Still ONE list, and both scorers still take their options from here — the
    guards in `tests/test_feed_market_type_load_only_1698.py` and
    `tests/test_feed_outcome_projection_cert622.py` pin both halves of that.

    The per-column notes (#1698 on `market_type`, L2-172 / UX-P011 on the
    outcome price fields, Q480 / CERT-622 on `external_id`, LAT-P195 on the
    image dimensions) travelled with the columns; the rules they state are
    unchanged and are why the list is a contract.
    """
    return _futures_snapshot.market_load_options()
...
async def _score_futures(
    ...
    # site and loaded at neither.
    #
    # LAT-P174 (#2143 residual): that factory now builds from
    # ``app/utils/futures_market_snapshot.py``, whose column tuples are ALSO the
    # wire format of the shared hydration artifact read below. So the query and
    # the snapshot cannot disagree about what is loaded, for the same reason the
    # two scorers cannot: there is one list, in one place.
    base_options = _futures_feed_load_options()
