...
# ── The twelve production boards the rule is FOR ─────────────────────────────
#
# Six labels are the date/Fed-chair boards #6508 names. The other seven are
# Polymarket tennis boards found only by censusing all 36,369 open markets —
# the population `_format_market_detail` actually serves. They are the same
# shape with a longer name: the repeated label is a PROP leg carried by two
# condition ids, one settled and one live. See `test_a_tennis_prop_leg_…` below.
...
def test_a_tennis_prop_leg_twin_is_the_same_ship_6508():
    """`61121801` W35 Shenyang — one of the seven boards the census added.

    The repeated label is the prop leg `… Set Handicap +/-1.5` under two
    condition ids: one settled at 0.000000, one live at 0.375000. The PLAYER
    rows are distinct names and are never grouped, so the two sides of the
    match both survive — that is what makes this the #6508 shape and not a
    settled-result deletion.
    """
    prop = "W35 Shenyang: Valeria Savinykh vs Yujia Huang Set Handicap +/-1.5"
    rows = [
        _row(1, "Valeria Savinykh", "0x93244ba2", 0.435),
        _row(2, prop, "0xd3b3ee29", 0.375),
        _row(3, prop, "0xc1722b83", 0.0, is_winner=False, resolution_source="api_settlement"),
        _row(4, "Yujia Huang", "0x11112222", 0.565),
    ]
    kept = _drop(rows)
    assert [r.id for r in kept] == [1, 2, 4]
    assert _names(kept).count("Valeria Savinykh") == 1
    assert _names(kept).count("Yujia Huang") == 1
...
# ── Guard 1, the arm the first version of this module got WRONG ──────────────
#
# Both boards below are all-settled groups containing a WINNER. The first
# spelling of the anchor test asked for a sibling that was "not a definite
# loss"; a settled winner satisfies that, so it anchored the group and the
# graded loser beside it was deleted. Measured on production over all 36,369
# open markets, that wrong spelling fired on five extra labels. These two tests
# are those five, and they are the reason the anchor test is `_is_unsettled`.


def test_a_settled_winner_cannot_anchor_a_drop_6508():
    """`61085841` Rennes: `… Set Handicap +/-1.5` under two condition ids.

    One graded win at 1.0, one graded loss at 0.0. Both are real results of a
    finished match, and nothing distinguishes which grade belongs to which
    question — guard 1 exactly. Same shape on `61110424`, `61135826`, `61141865`.
    """
    label = "Rennes: Maxime Janvier vs Sascha Gueymard-Wayenburg Set Handicap +/-1.5"
    rows = [
        _row(1, label, "0x2a4c0994", 1.0, is_winner=True, resolution_source="api_settlement"),
        _row(2, label, "0xb2d4fe58", 0.0, is_winner=False, resolution_source="api_settlement"),
    ]
    assert _drop(rows) == rows
    assert superseded_name_twins(rows, **ACCESSORS) == []


def test_a_settled_ladder_with_one_winner_keeps_every_rung_6508():
    """`109373` "How many Senators vote to confirm … as Chair" — the worst case.

    Ten rungs carry one name (their own threshold labels were lost upstream),
    nine graded losses and one graded win. Under the wrong spelling the single
    winner anchored the group and NINE graded rungs were dropped, leaving one —
    and `outcome_count` then reported a ten-rung ladder as a one-rung one.
    """
    label = "How many Senators vote to confirm  as Chair"
    rows = [_row(1, label, "KXSEN-1", 0.02, is_winner=True, resolution_source="api_settlement")]
    rows += [
        _row(i, label, f"KXSEN-{i}", 0.0, is_winner=False, resolution_source="api_settlement")
        for i in range(2, 11)
    ]
    assert len(_drop(rows)) == 10


def test_a_settled_but_ungraded_sibling_cannot_anchor_a_drop_6508():
    """An anchor needs NO verdict at all, not merely a verdict that isn't a loss.

    `resolution_source` set with `is_winner=None` is the venue having answered
    the market without this leg being graded. It is not a live question, so it
    cannot be the thing another row is superseded BY.
    """
    rows = [
        _row(1, "December 31", "0xaaa", 0.10, is_winner=None, resolution_source="api_settlement"),
        _row(2, "December 31", "0xbbb", 0.0, is_winner=False, resolution_source="api_settlement"),
    ]
    assert len(_drop(rows)) == 2
