...
class TestTheEsportsTournamentLeavesTheGolfPage:
    def test_the_name_side_gate_now_rejects_it_too(self):
        """WAS ``is True`` — the gap this file was written to expose, closed by
        the other lane while this branch was in flight.

        UX-P168 fixed the domain-neutral-title case on the FIELD side because the
        name side could not see it. Q446 landed on master afterwards and closed
        the same hole from the name side, with the GENERIC-WORD GATE: a market
        whose only golf signal is a word every sport owns ("masters", "open",
        "classic") must corroborate golf from something the classifier did not
        itself write — a declared tour, a Kalshi golf ticker, a named major, a
        golf market shape. "Asia Masters 2026 Winner" corroborates none of them,
        so it is rejected before the field is ever consulted.

        Asserted rather than deleted: this is the line that goes red if that
        corroboration requirement is ever relaxed back. The field-side gate below
        is now defence in depth for this specimen — and still the ONLY defence
        for its class, because a foreign field under a title that *does*
        corroborate golf would sail through the name side. See
        ``test_the_field_side_gate_drops_it``, which drives that helper directly.
        """
        assert _is_golf_market(_market(ESPORTS)) is False
