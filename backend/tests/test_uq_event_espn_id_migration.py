...
class TestItIsReachable:
    ...
    def test_the_model_declares_the_same_index_the_migration_builds(self):
        """Otherwise the next `--autogenerate` quietly undoes the invariant.

        `Event.espn_id` used to carry `index=True`, which is `ix_events_espn_id`
        — the loose index this migration DROPS. Left that way, the first
        autogenerated revision after this one lands sees the model asking for a
        non-unique index and the database holding a unique one, and dutifully
        writes a migration that drops `uq_events_espn_id` and re-adds the loose
        one. Nobody reviewing that diff would read it as "remove the invariant",
        which is exactly why it needs a test rather than a comment.
        """
        from app.models.models import Event

        by_name = {i.name: i for i in Event.__table__.indexes}
        assert "uq_events_espn_id" in by_name, (
            "the model does not declare the unique index; autogenerate will "
            "propose dropping it"
        )
        assert "ix_events_espn_id" not in by_name, (
            "the model still declares the loose index the migration drops; "
            "autogenerate will propose re-adding it"
        )

        index = by_name["uq_events_espn_id"]
        assert index.unique is True
        assert [c.name for c in index.columns] == ["espn_id"]
        # Partial, matching the migration's `postgresql_where`.
        where = index.dialect_options["postgresql"].get("where")
        assert where is not None and "espn_id IS NOT NULL" in str(where)
