...
# 🔴 CAL-P150 AMENDMENT TO A CAL-P143 PRE-BUILD, and it is the reason the
# red-first line in this queue's report reads `1` instead of `2`.
#
# `SoftStageOutcome` was imported at module scope beside the two names above.
# At the pre-D22 base that import does not resolve, so the FILE fails to
# collect: pytest exits 2, and per gotcha #124 a 2 is "the gate never ran", not
# a graded red. The four `_build_truth_evidence` tests in section 2 test a
# function that EXISTS at base and answers WRONG there — they are a genuine red
# and they were being thrown away by a collection error in a sibling section.
#
# So the new symbol is reached lazily, inside the one test that needs it. The
# three `soft_stage` tests still fail at base (PhaseRunner has no `soft_stage`),
# they now fail as tests rather than as a collection error, and nothing about
# what is asserted has changed.
def _soft_stage_outcome_cls():
    from app.tasks.calibration_main_build import SoftStageOutcome

    return SoftStageOutcome
