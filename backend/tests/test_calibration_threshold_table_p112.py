...
def test_every_class_has_a_bar_and_a_rationale():
    for klass in (tt.CLASS_A, tt.CLASS_B, tt.CLASS_C):
        assert klass in tt.PROPOSED_BARS
        assert tt.CLASS_RATIONALE[klass].strip()


def test_no_class_is_looser_than_the_reader_bar():
    """The proposal may hold a class TIGHTER than reader-actionability. It may
    ...
    for klass, bar in tt.PROPOSED_BARS.items():
...
def test_sigma_gate_keeps_an_indistinguishable_cell_out_of_the_queue():
    ...
    ev = tt.evaluate(tt.score(payload), tt.PROPOSED_BARS)
...
def test_class_A_bar_is_what_separates_the_two_tables():
    ...
    stops being true the proposal has become a relabelling, not a decision."""
    ...
    assert tt.evaluate(result, tt.PROPOSED_BARS)["cells_queued"] == 1
...
def test_needle_matches_the_spec_shape():
    ...
    ev = tt.evaluate(tt.score(payload), tt.PROPOSED_BARS)
    line = tt.needle(ev, "2026-08-28T17:33:03+00:00")
