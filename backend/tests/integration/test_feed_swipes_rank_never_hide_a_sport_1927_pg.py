...
async def _drop_tables_the_models_do_not_declare(conn):
    ...
    then fails with `DependentObjectsStillExistError`. Dropping by NAME would
    pin this file to another test's internals, so drop whatever `public` holds
    that the models do not declare. This fixture already drops every table it
    does know about; this is the same scope.
    """
