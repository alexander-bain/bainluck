...
class TestTheSpecimenStillFolds:
    """The pair folds because of lane1/293's `SOCCER_KICKOFF_DRIFT` (#5964), not
    because of anything in this ship. It is asserted here anyway: the carry
    below is only reachable while these two rows are still one card, so if that
    ever stops being true every assertion in the next class goes vacuous
    without going red. This is the non-vacuity guard for the rest of the file.
    """
    ...
    def test_the_la_liga_pair_two_minutes_apart_is_one_card(self):
        ...
        assert [e.id for e in result.events] == [15311881], (
            "the two Getafe rows are two cards again — the carry tests below "
            "are now vacuous "
