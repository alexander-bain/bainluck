...
class TestTableConfig:
    ...
    def test_sql_identifiers_are_static_safe_names(self):
        """Generated SQL interpolates identifiers, so configs must stay static."""
        safe_identifier = re.compile(r"^[a-z_][a-z0-9_]*$")
        for cfg in _TABLE_CONFIG.values():
            for key in ("table", "partition_col", "sub_partition_col", "value_col"):
                assert safe_identifier.fullmatch(cfg[key]), (key, cfg[key])
...
class TestGeneratedSQLSafety:
    """Guardrails for the SQL generated by _collapse_partition_sql."""

    def test_partition_sql_is_scoped_to_partition_and_cutoff(self):
        session = _RecordingSession()
        cutoff = datetime(2026, 2, 1, 12, 0, 0, tzinfo=timezone.utc)

        deleted, updated = asyncio.run(
            _collapse_partition_sql(session, _TABLE_CONFIG["odds"], 123, cutoff)
        )

        assert (deleted, updated) == (0, 0)
        assert len(session.executions) == 3
        for sql, params in session.executions:
            assert "FROM odds_snapshots" in sql
            assert "WHERE event_id = :part_id" in sql
            assert "AND captured_at < :cutoff" in sql
            assert params == {"part_id": 123, "cutoff": cutoff}

    @pytest.mark.parametrize("table_key", ["odds", "winprob", "futures"])
    def test_generated_sql_has_no_unbounded_destructive_statements(self, table_key):
        session = _RecordingSession()
        cutoff = datetime(2026, 2, 1, 12, 0, 0, tzinfo=timezone.utc)

        asyncio.run(_collapse_partition_sql(session, _TABLE_CONFIG[table_key], 123, cutoff))

        statements = [sql.upper() for sql, _params in session.executions]
        assert all("DROP " not in sql for sql in statements)
        assert all("TRUNCATE " not in sql for sql in statements)
        assert all("ALTER " not in sql for sql in statements)

        delete_statements = [sql for sql in statements if "DELETE FROM" in sql]
        assert len(delete_statements) == 1
        delete_sql = delete_statements[0]
