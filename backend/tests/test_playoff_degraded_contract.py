...
class TestTheFixturesAreStillWhatTheBackendProduces:
    ...
    def test_each_failure_reason_gets_its_own_phrase(self):
        # #2303 added a second way to 503 — a Postgres statement_timeout firing
        # below the route's 25 s wall. A reader told "timed out" when the
        # database cancelled the build is sent to the wrong place.
        from app.routes.playoffs import (
            GRID_FAILURE_DB_CANCELED,
            GRID_FAILURE_TIMEOUT,
        )

        assert "timed out" in degraded_grid_detail("epl", GRID_FAILURE_TIMEOUT)
        assert "cancelled by the database" in degraded_grid_detail(
            "epl", GRID_FAILURE_DB_CANCELED
        )
        # The default is the timeout wording, which is what the generated
        # fixture and every single-argument caller rely on.
        assert degraded_grid_detail("epl") == degraded_grid_detail(
            "epl", GRID_FAILURE_TIMEOUT
        )
        # The clause that carries the meaning survives BOTH severities.
        for reason in (GRID_FAILURE_TIMEOUT, GRID_FAILURE_DB_CANCELED):
            assert "not an empty league" in degraded_grid_detail("epl", reason)
...
class TestBothLayersAreStillWired:
    ...
    def test_the_route_raises_with_the_shared_producer(self):
        ...
        # The call gained its `reason` argument when #2303's DB-cancel path
        # merged into #1484's: `_serve_grid_degraded` is now the ONE raise site
        # for both failures, so it must pass the reason through rather than
        # re-typing a sentence per branch. The assertion is unchanged in force —
        # delete the call or inline the string and it still goes red.
        assert "detail=degraded_grid_detail(league_slug, reason)" in src, (
            "the 503 no longer raises through degraded_grid_detail(), so the "
            "sentence this module pins is not the sentence readers get"
        )

    def test_the_producer_is_the_only_place_the_sentence_is_built(self):
        # Non-vacuity for the merge itself: before it, `_serve_grid_degraded`
        # carried its own copy of this f-string. Two producers agreeing today is
        # the exact drift CERT-433 caught, so assert there is only one.
        src = PLAYOFFS_SRC.read_text()
        assert src.count("This is a degraded state, not an empty") == 1, (
            "the degradation sentence is built in more than one place again"
        )
