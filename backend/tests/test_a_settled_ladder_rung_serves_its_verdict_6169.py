...
class TestTheAxisIsTheRowsOwn:
    ...
    def test_a_losing_under_becomes_a_certain_over(self):
...
class TestTheTierBoundaryIsTheDocumentedOne:
    ...
    def test_the_row_shape_the_other_families_use_is_not_mistaken_for_a_verdict(self):
        ...
        assert not _row_price_is_a_verdict({"is_winner": False})
        assert not _row_price_is_a_verdict({"is_winner": True})
        assert not _row_price_is_a_verdict({})
        assert not _row_price_is_a_verdict(
