...
async def _restore():
    ...
    print("The door is still open — run --close if you also want it shut.")
    return 0
...
def main():
    ...
    if not args.dry_run and app_name is not None and app_name not in ALLOWED_APPS:
        p.error(f"refusing to act on HEROKU_APP_NAME={app_name!r}; "
                f"this script runs only on {ALLOWED_APPS}")
