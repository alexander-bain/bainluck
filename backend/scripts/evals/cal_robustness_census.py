...
its presence signals the run finished.
...
async def _write_marker(session, section: str, payload) -> None:
    """Persist one result section as a seed_diag marker row on ``entities``.

    Stores the JSON as a text blob under entity_metadata.payload (no 9000-char
    cap — this is our own writer), Core JSONB ``||`` merge (gotcha #4). Own
    ...
    existing = (
        await session.execute(select(Entity.id).where(Entity.external_ref == ref))
    ).scalar_one_or_none()
    if existing:
...
async def run() -> None:
    ...
    async with get_task_session() as session:
        # Never let a DB-side statement timeout kill the big scans (this is a
        # one-off dyno, off the web path — the whole point of the deferral).
        await session.execute(text("SET statement_timeout = 0"))

        progress: dict = {"started": True}
        await _write_marker(session, "progress", {**progress, "stage": "start"})
        ...
        """)))
        await _write_marker(session, "progress", {"stage": "games_monthly"})

        # --- Games: per-event density, joined to league/status -------------
        # One heavy GROUP BY read into Python (no temp tables — a mid-stream
        ...
        await _write_marker(session, "progress", {"stage": "game_ev_loaded", "n": len(game_ev)})
        ...
        await _write_marker(session, "games", {
        ...
        """)))
        await _write_marker(session, "progress", {"stage": "dedup"})
        ...
        """).bindparams(start=START)))
        await _write_marker(session, "progress", {"stage": "fut_ev_loaded", "n": len(fut_ev)})
        ...
        # Worst-100 genuinely-sparse futures: long open span (>=3d), few
        # effective pts (< 1 pt/day). Recoverability: polymarket -> CLOB
        # backfill; kalshi -> only within ~2-3mo of settlement (gotcha #35).
        def _recov(src):
            return {
        ...
        await _write_marker(session, "futures", {
        ...
        await _write_marker(session, "progress", {"stage": "futures_done"})
        ...
        # Compact drill-down: drop the per-cohort 10-example lists (huge), keep
        # the calibration stats.
        drill = [
            {k: c[k] for k in (
        ...
        await _write_marker(session, "sweep", {
        ...
        await _write_marker(session, "sweep_drill", {"drill_down": drill})
        await _write_marker(session, "progress", {"stage": "sweep_done"})

        # Final meta marker (written LAST — presence == run complete).
        await _write_marker(session, "meta", {
            "queue": "253",
            "start": START,
