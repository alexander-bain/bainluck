"""#6619 — the one-command undo for `repair_6619_half_verdicts_from_the_full_time_score.py` (D51).

Puts `is_winner` and `resolution_source` back for every outcome **that repair
...
exactly two columns, so the restore writes exactly those two back.
    ...
    python3 scripts/restore_6619_half_verdicts_from_the_full_time_score.py            # preview
    python3 scripts/restore_6619_half_verdicts_from_the_full_time_score.py --apply    # undo
...
CERT-2439 lesson, inherited from #4586 via #4923 rather than re-learned here.
...
a row the VENUE has settled since — and the undo would then replace Polymarket's
own verdict with the invented `game_score` one the repair existed to remove. The
manifest holds one row per SUCCESSFUL clear, so it is the only record of what
that script actually did.
...
THERE IS NO `--outside-declared-cohort` HERE, AND THE ABSENCE IS THE POINT.
#4923's restore needed that flag to rescue 70 correct verdicts its repair had
deleted, because that repair mirrored the period grammar into SQL and the mirror
matched a day-of-month (`KXMLSSPREAD-26APR22HOUSD` → `22H`). This repair
classifies by importing the producer's own `_ticker_period` / `_name_period`, so
there is no second copy of the grammar that can drift from the first and no
class of false positive for a flag to undo. If one is ever needed, the cohort
definition to negate is `repair_…6619….in_scope`, imported — never re-written.

A FULL UNDO IS NOT THE EXPECTED USE. The verdicts this restores are the ones
production was measured to have no evidence for (0 of 130 cohort events hold a
period linescore or a `scoring_plays` row), so putting them back re-prints
claims we cannot support. It exists because D51(b) requires an unattended repair
to ship one, and for the narrow case where the repair must be rolled back as a
UNIT — a bad run, an interrupted apply, a decision reversed. Reach for `--limit`
on the repair before reaching for this.
...
#: Every outcome the repair cleared, with what the live row says NOW and what
...
async def run(args) -> None:
    ...
    async with get_task_session() as s:
        ...
        print(f"=== #6619 restore plan: {len(rows)} outcomes in the manifest ===")
        ...
        for r in moved_on[:10]:
            print(f"    {r.outcome_id:>10} now {r.current_resolution_source} "
                  f"win={r.current_is_winner} | {(r.market_name or '')[:44]}")
        if moved_on:
            print("  (those are the venue's own verdicts arriving — the repair "
                  "working, not failing.)")

        if not args.apply:
            print("\nPREVIEW — no writes. Pass --apply to undo.")
            return
...
if __name__ == "__main__":
    ...
    p.add_argument("--apply", action="store_true",
                   help="write the pre-repair verdicts back (default is preview)")
