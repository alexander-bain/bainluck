...
3. 🟡 **Data retention policy** — DB is at 571MB / 1GB (56%), ~2M rows in `odds_snapshots`, ~1M in `futures_odds_snapshots`. Heroku plan: essential-0 (1GB cap, no row limit). **Phase 1 (quick win):** Prune pre-game snapshots for completed events — delete snapshots captured before `commence_time`. These are low-value (line barely moves) but high-volume (2-5 min polling × 5-11 bookmakers × hours/days). **Phase 2 (later):** Aggregate in-game snapshots into 1-per-minute medians in `odds_aggregated` table (schema exists, never populated), then prune raw in-game snapshots for events older than 7 days. This preserves OddsChart, Pulse recalculation, and Hall of Fame functionality. ~93% row reduction per event.
