...
35. **Kalshi API retains settlement data permanently** — `GET /events/{ticker}` returns `result` for settled events of any age. Never add time-window filters assuming old data is purged. The API has 100% hit rate across all ages when requests aren't silently dropped by error handling.
