...
54. **A sharp boundary in a paginated read is evidence about the READER before it is evidence about the system** — the sibling of #53: there, an empty response was misread as a fact; here, a *truncated* one is. GitHub's Project v2 `items` (and most cursor-paginated APIs) return one page unless you follow `pageInfo.hasNextPage`. Project 1 holds ~1,278 items across 13 pages, so a single unpaginated call returns ~100 whose highest open issue lands right about **#1243** — and that page edge was read as a clean date cutoff, i.e. "a mechanism stopped". It produced three separate wrong numbers ("238 of 416 open issues off-board", "346 missing", "the board's highest item is #1243"), all from one call, and they were confident enough to get a whole queue staged on a defect that did not exist. The true figure was **0 off-board**. **Never infer a boundary, a gap, or a "stopped on date N" from a read you cannot prove was complete**: paginate, then assert `fetched == totalCount` and RAISE on mismatch — a short read must never be reported as a gap (which invents work) nor as a pass (which hides it). `app/tasks/board_sync.py::verify_complete_read` is the reference shape, and its first regression test is this incident.
