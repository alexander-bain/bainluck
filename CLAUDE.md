...
35. 🟢 **Golf landing page (shipped)** — Bespoke category page at `/categories/golf` with cross-source tournament odds aggregation, current event detection, 24h movers, sparkline charts, LPGA/TGL separation, non-golf false positive filtering. Post-launch fixes: clean slugs (strip sponsor suffixes), importance-aware current event detection (Signature Events tier), clickable upcoming events, tournament link discoverability. See Golf Landing Page section above.
...
18. **🚨 The Odds API quota is CRITICAL — monitor constantly**: The monthly quota is 5M "billed requests" (~$119/mo plan). The API bills per `events_returned × market_types × regions`, NOT per HTTP call. With `markets=h2h,spreads,totals` (3) and `regions=us,us2` (2), **every event in a response costs 6 billed requests**. A sport with 12 events = 72 billed requests per API call. **Mitigation (shipped Feb 2026):** Tiered discovery frequency gates tier 3-4 sports to 2-4h intervals instead of 15min, saving ~53% of discovery API calls (~1.9M billed requests/month). Expected daily burn rate post-mitigation: **~90-120K billed requests/day** (was ~150-250K). **Before making ANY changes to polling frequency, sport list, market types, or regions, calculate the impact on the monthly quota.** Check current usage with `curl https://api.bainluck.com/health/ready` (look at `odds_api.requests_remaining`). If remaining < 500K, alert the user immediately. There is currently NO automated alerting — this must be added. The quota resets on the 1st of each month. Running out means zero event discovery, zero odds updates, and zero futures polling until the reset. Feb 2026 exhausted the full 5M quota by ~Feb 26.

19. **`event.period` can contain clock prefixes**: ESPN sync stores `status_detail` as `event.period`, which is usually just `"1st Quarter"` or `"OT"`. But the field can sometimes contain `"6:55 - 1st Quarter"` (clock + period). Any code parsing `event.period` must strip the `"clock - "` prefix first. Both `_parse_game_progress()` in `highlights.py` and `parse_game_clock()` in `win_probability.py` handle this — if you add new period-parsing code elsewhere, do the same.
