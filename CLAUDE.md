...
| `docs/doctrine.md` | The GENERAL clauses lifted out of rulings — the sentence that survives deleting its case (ruling 081). Rulings stay narrow and keep their evidence; the clause that explains a second, unrelated case is routed here. Not ordering, not status. | When a ruling contains a clause that pays out outside its own case |
...
- **Deployments from GitHub**: `git push origin master` triggers CI; Vercel deploys frontend from GitHub, and Heroku deploy runs through the serialized CI `deploy` job after tests pass (that job's `git push heroku master --force` transport is live — see gotcha #21). **But pushing master is not a step you take because you finished the work.** Under Program Lanes the Integrator alone rebases, gates, merges, pushes and verifies master, and ruling 017 requires holding `.claude/handoff/LANE-integrator.lock` for the push in **any** lane that writes master, `/triage` included. Gates prove something about the commit you tested, not the commit you push.
...
- **Frontend build (MANDATORY before push)**: `cd frontend && npm run build` — this is the **ESLint gate** (rules-of-hooks, etc.); Vercel runs this exact command. `next.config.mjs` sets `typescript.ignoreBuildErrors: true`, so the build does **NOT** fail on TS *type* errors.
- **Frontend typecheck (MANDATORY before push)**: `cd frontend && npm run typecheck` — the **TypeScript gate**, and a real deploy gate in CI since L2-234. Fail-on-new against `frontend/typecheck-baseline.json`: it fails on one error more than the baseline **and** on one fewer (fix errors → `npm run typecheck:baseline` to lower it). Run it **after** `npm run build`, since the build generates the `.next/types/**` that tsconfig includes. See gotcha #10.
...
**Prediction Market Matching** (`tasks/prediction_market_matching.py`): Runs **every 15 minutes** (beat: `crontab(minute="5,20,35,50")`) and links Kalshi/Polymarket game markets to events. (This said "hourly" until 2026-08-21 while the same file said 15 min forty lines further down — C-DOC-DRIFT-1; the beat schedule is the authority.) Three-phase: Link (Pass 1 ticker scan + Pass 2 general scan) → Re-validate (Phase 1.5) → Snapshot writing (Phase 2). Per-market commit to avoid deadlocks with live polling task. Link rate tracked at `/api/admin/prediction-markets/link-rate`.
...
| **Query plan** | same endpoint, `{"sql":"SELECT ...","explain":true}` → `EXPLAIN (FORMAT JSON)`. Supply a plain SELECT — the server composes the EXPLAIN; typing `EXPLAIN ...` yourself is still rejected. Does **not** execute, so it works on queries that always hit the timeout. `"analyze":true` DOES execute (SELECT-only, no leading `WITH`) and since LAT-P027 additionally requires every function call to be on the pure-function allowlist in `app/utils/sql_read_guard.py` — an unlisted name is refused BY NAME, so widening it is a one-line change. Bound via `"timeout_ms"` (500 ms–25 s, default 10 s) |
| **db-query executes, so it refuses operational functions** | On BOTH the row path and `analyze` — `pg_cancel_backend`, `pg_terminate_backend`, the advisory locks, `nextval`/`setval`, `pg_sleep`, `dblink`, `pg_read_file` are refused before reaching the database. `SET TRANSACTION READ ONLY` does **not** make these safe: it forbids writes to tables and sequences, and cancelling a backend is neither (#1641). Plan-only (`explain:true` without `analyze`) is deliberately NOT allowlisted, because it does not execute. Responses no longer echo your SQL — `explain_sql` was replaced by `explain_mode` + `sql_fingerprint`, plans are capped at 256 KiB with an explicit `truncated` verdict, and errors return `{reason, correlation_id}` instead of the database's message |
