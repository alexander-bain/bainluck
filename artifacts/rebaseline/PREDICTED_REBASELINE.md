...
|---|---|---|---|---|---|
| **FLAG-1** | `kalshi/entertainment/quantity` (rank 11, 20.11pp) | Queue row 2: cumulative ladders `−3 to −6pp` on `quantity 40–50% ladder 14.18→~8pp` (polymarket book ladders); no explicit kalshi prediction — implicit `similar 3–6pp`, so `20.11 → ~8–11pp` if book vig assumed. | **13.8–17.1pp** sim vs **8–11pp** queue (sim stays 13.8 worst-case) | **disagree 5–9pp** | **Verdict: QUEUE-WRONG** — queue transplanted Polymarket's *book* vig (`remove_vig_nway` on `1.2+` per-rung overround) onto Kalshi's *exchange* price (`kalshi` is `price = last trade`, no book overround; `odds_math.py:47` never runs on `kalshi`). Kalshi `entertainment quantity` n=1200 is likely `exclusive_ranges` (e.g., `0-5 / 6-10` viewers) not `cumulative_thresholds` (`Over N`), but even as cumulative the per-rung `YES+NO` sum on Kalshi is `1.00–1.03` (exchange spread) not `1.15–1.25` (book). The `−3 to −6` bound is therefore *generous* for Kalshi — real per-rung de-vig is `−0.5 to −1.5pp`. **Resolved predicted: 15.5–18.5pp** (sim's low end `13.8` was optimistic; queue's `8` assumed book vig on exchange). Discriminator: `SELECT outcome_relation, AVG(yes_no_sum) FROM per_rung WHERE fm.source='kalshi' AND market_type='quantity'` — expect `1.01` vs `1.20` on polymarket. If build shows `≤10pp`, queue underestimated exchange+field completeness, not book vig. |
| **FLAG-2** | `polymarket/baseball/field` (rank 10, 20.13pp) | Queue row 2: exclusive cells after de-vig `1.5+` bucket empties → `−5 to −15pp` on exclusive where `sum 2–5` (field containers, the `members=5 → median 2.5` specimen). Implies `20.13 → ~5–10pp`. | **11.2pp** sim (point `−8.5` exclusive) | **Δ 1–6pp** vs low end `−15 → 5pp GREEN` | **Verdict: QUEUE-WRONG** — queue's `−15` best-case was measured on `container_member` 5-member containers (`precompute_calibration.py:262` `sum 2.5` on `polymarket:{event.id}` groups, `admin_cohort.py:330` `COALESCE(group_id,event_id)`). Baseball `field` is `q≈n` single-winner partitions (`market_shape.py` field, `market_type='field'` alone, not `group_id` container) where many fields are *incomplete* (`field_is_complete_for_normalization` requires `survivor_n==eligible_n && survivor_n>=3 && cp_sum>1.15`, `:631:650`). Incomplete fields are *excluded* from n-way normalization, not forced to `sum=1`, so effective correction is `−6 to −9pp` not `−15`. Sim's `−8.5` already assumes `field` completeness holds; queue's `−15` assumed every baseball field is a complete 5-20 runner partition. **Resolved predicted: 11.2pp stays** (queue corrected to `10–13pp`, not `5pp`). Discriminator: `SELECT COUNT(*) FILTER (WHERE survivor_n != eligible_n) / COUNT(*) AS incomplete_share FROM field_completeness WHERE source='polymarket' AND league='baseball'` — expect `0.4–0.6` incomplete, explaining why `−15` cannot be realized. |
| **FLAG-3** | `polymarket/soccer/quantity` (rank 3, 44.66pp) | Queue row 1: worst `quantity 50.00pp → 5–10pp` if collapse (provenance) else ≤5pp if survive. Implies rank 3 collapses to `5–10pp`. | **6.5–9.8pp** | **within** queue range | **Verdict: SEMANTICS — both right, near-miss recorded.** `graded_share 0.27` vs table_tennis `0.18` makes collapse shallower; queue's `5–10` already brackets `6.5–9.8`. No table change, but row is the sensitivity canary: if `null_default_share` is `0.55` not `0.73`, build will be `~12pp` and will become a new flag post-hoc. Keep as written. |
| **FLAG-4** | `kalshi/economics/quantity` (rank 20, 7.85pp) | Queue has no registered prediction for kalshi economics quantity at `7.85pp` — implicit “ladder cohorts `2–4pp` lower” → `~4pp` point. Sim: **1.5–4.8pp** (range straddles GREEN) | **1.5–4.8pp** (best `1.5` GREEN vs worst `4.8` GREEN) | **Δ up to 2.5pp** vs point | **Verdict: SEMANTICS — rule interval, both right under different definitions.** Queue used point `ECE` (`expected_calibration_error` outcome-weighted, `cohort_sweep.py:334`), sim used bounded interval (`Fix 2` cumulative `[−3,−6]` + `Fix 6` `0.1pp` band epsilon + `Fix 5` timezone). A cell is **GREEN only if its entire interval is `≤5.0pp`** (i.e., upper bound `≤5pp`), otherwise it is `RED` (upper >5) or `NOT-PROVABLE` (graded_share<0.5 wins before GREEN). Under that rule the cell is **not GREEN in the worst case** (`4.8pp` is GREEN but only just, and `1.5–4.8` means the build must clear `4.8pp`, not `1.5pp`). **Resolved predicted: 2.2–4.8pp** (tighten low end `1.5→2.2` after removing spurious `Fix 6` double-count on non-`0.50` band outcomes; definition ruled as interval GREEN). |
