...
WITH base AS (
  SELECT o.market_id, o.name, o.current_probability AS p,
         o.current_yes_bid AS b, o.current_yes_ask AS a, m.name AS mname, m.source, m.mutually_exclusive AS me
  ...
  SELECT *,
    ...
    (me AND wide AND (b IS NULL OR b<=0.05) AND p>0.05 AND p=top_p) AS lead_hit
  FROM o
), agg AS (
