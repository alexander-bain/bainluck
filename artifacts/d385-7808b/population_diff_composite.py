...
WITH base AS (
  ...
  SELECT market_id, MAX(mname) AS mname, MAX(source) AS source,
         ...
         COUNT(*) AS legs, SUM(p) AS psum,
         COUNT(*) FILTER (WHERE buyer) AS buyers
  FROM f GROUP BY market_id
), hit AS (
  SELECT f.market_id, f.name, f.p, f.b, f.a, agg.mname, agg.source,
         agg.best_bid_p, agg.buyers, agg.legs, agg.psum
...
"""
Q1 = CORE + "SELECT COUNT(DISTINCT market_id) AS markets_touched, COUNT(*) AS legs_hidden FROM hit"
Q2 = CORE + "SELECT market_id, source, mname, name, p, b, a, best_bid_p, legs, psum FROM hit ORDER BY p DESC LIMIT 30"
def run(sql, limit):
...
if "rows" in d2:
    for r in d2["rows"]:
        ...
        print(f"{m['market_id']:>9} {m['source'][:10]:11}{str(m['mname'])[:38]:40} HIDE {str(m['name'])[:20]:21} p={m['p']} b={m['b']} | best-bid p={m['best_bid_p']} legs={m['legs']} sum={float(m['psum']):.2f}")
else: print(d2)
