...
WITH o AS (
  SELECT o.market_id, o.name, o.current_probability AS p,
         ...
         m.group_type, m.source, m.mutually_exclusive AS me,
         ((o.current_yes_bid IS NOT NULL OR o.current_yes_ask IS NOT NULL)
          AND (COALESCE(o.current_yes_ask,1.0) - COALESCE(o.current_yes_bid,0.0)) >= 0.2) AS wide
  ...
  SELECT *,
    ...
    ((wide AND (b IS NULL OR b <= 0.05) AND ABS(p - (COALESCE(b,0.0)+COALESCE(a,1.0))/2) < 0.0005) OR (me AND wide AND (b IS NULL OR b <= 0.05) AND p > 0.05)) AS ph_new,
